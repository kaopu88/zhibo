;(function ($) {
  ...
  $.fn.circleChart = function (options) {
    const defaults = {
      color: 'rgba(244, 229, 209,1)',
      backgroundColor: 'rgba(244, 229, 209,0.2)',
      background: true,
      speed: 2000,
      widthRatio: 0.2,
      value: 66,
    ...
    Circle.prototype = {
      onDraw(el) {
        if (this.settings.onDraw !== false) {
          ...
          copy.value = (units[this.settings.unit] || units['default'])(
            copy.cAngle
          )
      ...
      drawBackground(ctx) {
        ...
        ctx.arc(
          this.pos,
          this.pos,
          this.settings.backgroundFix ? this.radius * 0.9999 : this.radius,
          0,
          2 * Math.PI
        )
      ...
      animate(el, ctx, time, startTime, move /*move counterclockwise*/) {
        ...
        if (
          time - startTime < this.settings.speed * 1.05 /* time not over */ &&
          ((!move &&
            this.cAngle * 1000 <=
              Math.floor(this.eAngle * 1000)) /* move clockwise */ ||
            (move &&
              this.cAngle * 1000 >=
                Math.floor(this.eAngle * 1000))) /* move counterclockwise */
        ) {
          this.cAngle = Math[this.settings.animation](
            (time - startTime) / mspf,
            this.sAngle,
            this.eAngle - this.sAngle,
            this.settings.speed / mspf
          )
