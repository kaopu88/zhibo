...
class KpiRedis
{
    ...
    public function __construct($now = null, $redisConfig = [])
    {
        ...
        $this->redis = isset($redisConfig['redis']) ? $redisConfig['redis'] : RedisClient::getInstance();
    ...
    public function dayInc($num = 0)
    {
        ...
        $res = $this->redis->zIncrBy($dKey, $num, $this->userId);//这是增量
         $mKey = $this->getMonthKey();
        $res = $this->redis->zIncrBy($mKey, $num, $this->userId);
        //只统计日不再累加到月2018-11-06
