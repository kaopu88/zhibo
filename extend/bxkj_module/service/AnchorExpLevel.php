...
class AnchorExpLevel extends Service
{
    public static function getAnchorLevelInfo($level, $default = null)
    {
        if (!isset($default)) {
            $default = array(
                'levelid' => '',
        ...
        $key = 'anchor:level:list';
        $expLevels = cache($key);
        if (empty($expLevels) || config('app.app_debug')) {
            $expLevels = Db::name('anchor_exp_level')->field('levelid,name level_name,level_up,icon level_icon')->select();
