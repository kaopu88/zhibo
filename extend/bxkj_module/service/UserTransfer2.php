...
class UserTransfer extends Service
{
    ...
    protected function handlerOne($tmpUser)
    {
        ...
        //以前的绑定关系
        $previous = Db::name('promotion_relation')->where(['user_id' => $user['user_id'], 'agent_id' => $this->target['agent_id']])->find();

        if (!empty($previous))
        {
            $oldPromoterUid = (int)$previous['promoter_uid'];
            $oldAgentId = (int)$previous['agent_id'];
        ...
        if (!empty($previous))
        {
            $delNum = Db::name('promotion_relation')->where(['id' => $previous['id']])->delete();
            ...
            //上一个推广员的客户数量递减1
            if (!empty($previous['promoter_uid'])) Db::name('promoter')->where(['user_id' => $previous['promoter_uid']])->setDec('client_num', 1);
        }
