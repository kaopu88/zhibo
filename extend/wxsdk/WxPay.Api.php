...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            throw new WxPayException("异步通知地址不能为空");
        }
    ...
    {
        $wxConfig = self::getConfig("wxpay");
        //如果不需要上报数据
        if ($wxConfig['report_levenl'] == 0) {
            return;
        }
        //如果仅失败上报
        if ($wxConfig['report_levenl'] == 1 &&
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        //如果有配置代理这里就设置代理
        if ($wxConfig['wxpay_proxy_host'] != "0.0.0.0"
            && $wxConfig['wxpay_proxy_port'] != 0
        ) {
            curl_setopt($ch, CURLOPT_PROXY, $wxConfig['wxpay_proxy_host']);
            curl_setopt($ch, CURLOPT_PROXYPORT, $wxConfig['wxpay_proxy_port']);
        }
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, $wxConfig['wxpay_public_cert']);
            curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($ch, CURLOPT_SSLKEY, $wxConfig['wxpay_private_cert']);
        }
        //post提交方式
