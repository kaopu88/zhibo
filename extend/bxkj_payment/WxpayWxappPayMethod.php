...
class WxpayWxappPayMethod extends WxpayPayMethod
{
    protected $payMethod = 'wxpay_wxapp';

    public function __construct($myAppKey = 'user')
    {
        parent::__construct();
        $this->config = config("payment.wxpay_wxapp");
    }
    ...
    public function checkNotify($post)
    {
        ...
        $sign = $post['sign'];
        unset($post['sign']);
        $wxConfig = config("payment.wxpay_wxapp");
