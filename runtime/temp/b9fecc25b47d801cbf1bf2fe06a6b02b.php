            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <div style="float: left">
                        <?php if(check_auth('admin:user_credit_rule:add',AUTH_UID)): ?>
                            <a href="<?php echo url('add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a>
                        <?php endif; if(check_auth('admin:user_credit_rule:delete',AUTH_UID)): ?>
                    ...
                    <div class="filter_search">
                        <input placeholder="规则ID、规则名称" type="text" name="keyword" value="<?php echo input('keyword'); ?>"/>
                        <button class="filter_search_submit">搜索</button>
                    </div>
                    <div class="clear"></div>
                </div>
            </div>
            <input type="hidden" name="change_type" value="<?php echo input('change_type'); ?>"/>
        </div>
        ...
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <?php switch($vo['change_type']): case "inc": ?><span class="fc_green"><?php break; case "exp": ?><span class="fc_red"><?php break; endswitch; ?>
                            <?php echo htmlentities($vo['change_type_str']); ?>
                            </span>
                        </td>
                        <td><?php echo htmlentities($vo['full_value']); ?></td>
                        <td><?php echo htmlentities($vo['full_score']); ?></td>
                        <td><?php echo htmlentities($vo['tpl']); ?></td>
                        <td><?php echo htmlentities($vo['admin']); ?></td>
                        <td><?php echo htmlentities($vo['day_max']); ?></td>
                        <td>
                            <?php if(check_auth('admin:user_credit_rule:update',AUTH_UID)): ?>
