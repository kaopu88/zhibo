<?php /*a:1:{s:55:"/www/wwwroot/zhibb/application/h5/view/invite/index.tpl";i:1691734333;}*/ ?>
<!doctype html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title><?php echo htmlentities($product_slogan); ?> 邀请您体验<?php echo htmlentities($product_name); ?>服务~</title>
    ...
    <link href="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/css'); ?>/h5_show/all.css?v=<?php echo config('upload.resource_version'); ?>" type="text/css" rel="stylesheet"/>
    <link href="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/css'); ?>/h5_show/index.css?v=<?php echo config('upload.resource_version'); ?>" type="text/css" rel="stylesheet"/>
    ...
    <script type="text/javascript">
        var __HOME = '/static/h5', globleLoadingImg = [], defaultPath = '';
        var userId = '<?php echo htmlentities($user['user_id']); ?>';
        var anchorId = '<?php echo htmlentities($anchor['user_id']); ?>';
        var sendSmsCodeUrl = '<?php echo url("common/send_code"); ?>';
        var countdown = parseInt('<?php echo htmlentities($countdown); ?>');
        var regUrl = '<?php echo url("invite/reg"); ?>';
        var wxSuccess = '<?php echo htmlentities($wx); ?>';
        var downloadUrl = '<?php echo url("h5/download/index"); ?>';
    </script>
    <style type="text/css">
        .touxiang {
            border-radius: 120px;
    ...
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/xmt.js?v=<?php echo config('upload.resource_version'); ?>"></script>
</head>
...
<div class="weixin-tip">
    <p>
        <img src="/static/common/image/default/share_friend.png" alt="微信打开" style="max-width: 100%;height: auto;"/>
    </p>
</div>
    ...
    <section class="main-page current">
        <div class="poa-wh">
            <div class="font_title" style="width: 100%">
                ...
                <div class=""><?php echo htmlentities($product_name); ?></div>
            </div>
            ...
            <div class="offset">
                <div style="position: relative;" id="yingcang1">
                    <div class="bg_sm">
                        <div class="write1">
                            <img src="<?php echo htmlentities($info['avatar']); ?>" class="touxiang"/>
                            <div class="font_info">
                                Hi~，我是来自<?php echo htmlentities($product_name); ?>的<?php echo htmlentities(short($info['nickname'],15)); ?>，<?php echo htmlentities($product_name); ?>是一款专注于年轻人的原创短视频社交分享应用。一起发现更多精彩吧~
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg" id="yingcang3">
                        ...
                        <?php if($bean > '0'): ?>
                            <div class="detail" id="yingcang2">
                                <p>领取<?php echo htmlentities($bean_name); ?>后下载<?php echo htmlentities($product_name); ?>APP即可使用；</p>
                                <p><?php echo htmlentities($bean_name); ?>可以自存也可以用来购买礼物砸晕你心仪的主播；</p>
...
<script src="http://res.wx.qq.com/open/js/jweixin-1.2.0.js?v=<?php echo config('upload.resource_version'); ?>"></script>
<script>
    var wxConfigJson = '<?php echo htmlspecialchars_decode($jsapi_config); ?>', wxConfig = {};
    if (wxConfigJson != '') {
        try {
...
<script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/invite.js?v=<?php echo config('upload.resource_version'); ?>"></script>
...
<div class="popup_phone_box popup_box">
    ...
    <ul class="popup_list">
        ...
        <li style="position: relative; <?php if($site['invite_code'] == '0'): ?> display:none <?php endif; ?> ">
            <input name="invite_code" placeholder="邀请码" class="popup_input" value="<?php echo htmlentities($user['invite_code']); ?>"/>
        </li>
        ...
        <li style="font-size: 12px;color: #888;line-height: 22px;">
            * <span style="font-size: 12px;color: #888;line-height: 22px;" class="popup_act"></span>成功后即可使用手机号登录APP<?php echo $bean>0?('，登录后将自动领取'.$bean.$bean_name.'奖励。'):'。'; ?>
        </li>
    </ul>
</div>
...
<div class="popup_account_box popup_box">
    ...
    <ul class="popup_list">
        ...
        <li style="font-size: 12px;color: #888;line-height: 22px;">
            * <span style="font-size: 12px;color: #888;line-height: 22px;" class="popup_act"></span>成功后即可使用用户名密码登录APP<?php echo $bean>0?('，登录后将自动领取'.$bean.$bean_name.'奖励。'):'。'; ?>
        </li>
    </ul>
</div>
