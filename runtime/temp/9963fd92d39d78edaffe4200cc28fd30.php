...
<head lang="en">
    ...
    <script>
        var reReviewUrl="<?php echo url('re_review'); ?>";
        var myConfig = {
            list: [
                {
                    name: 'status',
                ...
                },
                {
                    name: 'topic',
                    title: '视频话题',
                    get: '<?php echo url("video/get_topics"); ?>'
                },
                {
                    name: 'province',
                    title: '所在省份',
  ...
  <div class="sub_catalog"></div>
    ...
    <div class="pa_20">
        ...
        <div class="filter_box mt_10">
                ...
                <div class="filter_order">
                    <div style="float: left">
                        <?php if(check_auth('admin:film:add',AUTH_UID)): ?>
                            <a class="base_button base_button_s" target="_blank" href="<?php echo url('video/batchadd'); ?>">上传视频</a>
                        <?php endif; if(check_auth('admin:film:change_status',AUTH_UID)): ?>
                            <div ajax="post" ajax-url="<?php echo url('video/change_status',['status'=>'1']); ?>"
                                 ajax-target="list_id"
                                 class="base_button base_button_s base_button_gray">上架
                            </div>
                            <div ajax="post" ajax-url="<?php echo url('video/change_status',['status'=>'0']); ?>"
                                 ajax-target="list_id"
                                 class="base_button base_button_s base_button_gray">下架
                            </div>
                            <?php if(check_auth('admin:film:delete',AUTH_UID)): ?>
                                <a href="<?php echo url('video/del'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a>
                            <?php endif; endif; ?>
                    </div>
            ...
            <input type="hidden" name="free_status" value="<?php echo input('free_status'); ?>"/>
            <input type="hidden" name="audit_status" value="<?php echo input('audit_status'); ?>"/>
            <input type="hidden" name="visible" value="<?php echo input('visible'); ?>"/>
            <input type="hidden" name="source" value="<?php echo input('source'); ?>"/>
            <input type="hidden" name="sort" value="<?php echo htmlentities($get['sort']); ?>"/>
            <input type="hidden" name="sort_by" value="<?php echo htmlentities($get['sort_by']); ?>"/>
        </div>
        <div class="table_slide">
        <table class="content_list mt_10" style="table-layout: fixed;">
            <thead>
            <tr>
       ...
       class="thumb_img" style="display: inline-block;max-width: 100px;">
        <img src="<?php echo img_url($vo['animate_url']?$vo['animate_url']:$vo['cover_url'],'120_68','film_cover'); ?>"/>
                        ...
                        <td>
                            ...
                            <?php endif; ?>
                            <p class=""><?php echo htmlentities($vo['describe']); ?></p>
                        ...
                        <td>
                            <?php if(!(empty($vo['goods']) || (($vo['goods'] instanceof \think\Collection || $vo['goods'] instanceof \think\Paginator ) && $vo['goods']->isEmpty()))): ?>
                                ...
                                <Br>
                                <?php echo htmlentities($vo['goods']['short_title']); ?>
                                <span style="color: #8c8c8c">
                                    <?php switch($vo['goods']['shop_type']): case "B": ?>来自天猫<?php break; case "C": ?>来自淘宝<?php break; case "P": ?>来自拼多多<?php break; case "J": ?>来自京东<?php break; case "Z": ?>来自自营<?php break; endswitch; ?>
                                </span>
                                <?php else: ?>
                                暂无
                            <?php endif; ?>
    ...
    <p class="thumb_info">
        <a href="<?php echo url('user/detail',['user_id'=>$vo['user']['user_id']]); ?>">
            ...
            <?php echo htmlentities((str_hide($vo['user']['phone'],3,4) ?: '未绑定')); ?>
                        ...
                        <td>
                            播放：<?php echo htmlentities($vo['play_sum']); ?> <a poplink="play_sum" data-query="id=<?php echo htmlentities($vo['id']); ?>" href="javascript:;">调整</a><br/>
                            下载：<?php echo htmlentities($vo['down_sum']); ?><br/>
                            总赞：<?php echo htmlentities($vo['zan_sum']); ?> <span class="fc_gray">(<?php echo htmlentities($vo['sco_zan_sum']); ?>)</span> <br/>
                            虚拟赞：<?php echo htmlentities($vo['zan_sum2']); ?>  <a poplink="zan_sum2" data-query="id=<?php echo htmlentities($vo['id']); ?>" href="javascript:;">调整</a><br/>
                            评论：<?php echo htmlentities($vo['comment_sum']); ?> <span class="fc_gray">(<?php echo htmlentities($vo['sco_comment_sum']); ?>)</span> <br/>
                            人次：<?php echo htmlentities($vo['watch_sum']); ?><br/>
                            完播：<?php echo htmlentities($vo['played_out_sum']); ?><br/>
                            切换：<?php echo htmlentities($vo['switch_sum']); ?>
                        </td>
...
<?php endif; ?>
                        ...
                        <td>
                            <?php if($vo['visible'] == '2'): ?>
                                私密视频
                                <?php else: if($vo['status_abnormal'] == '1'): ?>
                                    <span class="fc_red">状态异常</span>
                                    <a href="javascript:;">修复</a>
                                    <br/>
                                <?php endif; ?>
                                <div tgradio-not="<?php echo check_auth('admin:film:change_status')?'0':'1'; ?>" tgradio-on="1"
    ...
    <?php break; case "3": ?>
        ...
        <?php endif; break; endswitch; ?>
                            <br/>
                            <?php if(!(empty($vo['audit_admin']) || (($vo['audit_admin'] instanceof \think\Collection || $vo['audit_admin'] instanceof \think\Paginator ) && $vo['audit_admin']->isEmpty()))): ?>
                        ...
                        <td>
                            <?php if(check_auth('admin:film_comment:select',AUTH_UID)): ?>
                                <a href="<?php echo url('video_comment/_list',['video_id'=>$vo['id']]); ?>">评论(<?php if(!(empty($vo['comment_sum']) || (($vo['comment_sum'] instanceof \think\Collection || $vo['comment_sum'] instanceof \think\Paginator ) && $vo['comment_sum']->isEmpty()))): ?><?php echo htmlentities($vo['comment_sum']); else: ?>0<?php endif; ?>)</a><br/>
                            <?php endif; if(check_auth('admin:recommend_content:rec_film',AUTH_UID)): ?>
                                <a poplink="recommend_box" data-query="id=<?php echo htmlentities($vo['id']); ?>&type=film" href="javascript:;">推荐(<?php echo htmlentities($vo['rec_num']); ?>)</a><br/>
                            <?php endif; if(check_auth('admin:film:update',AUTH_UID)): ?>
                                <a data-id="id:<?php echo htmlentities($vo['id']); ?>" poplink="film_audit_update" href="javascript:;">编辑</a><br/>
                            <?php endif; if(check_auth('admin:film:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get"
                                   href="<?php echo url('del',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
...
<div title="短视频编辑" class="layer_box film_audit_update pa_10" dom-key="film_audit_update"
    ...
    <div dom-key="zan_sum2" popbox="zan_sum2"  class="zan_sum2 layer_box pa_10" title="推荐" popbox-action="<?php echo url('video/edit_zan_sum2'); ?>" popbox-get-data="<?php echo url('video/edit_zan_sum2'); ?>" popbox-area="560px,450px">
        虚拟点赞数: <input class="base_text_s" name="zan_sum2"  value=""/>
         <input type="hidden" name="id"  value=""/>
        <div class="base_button sub_btn">保存</div>

</div>
    <div dom-key="play_sum" popbox="play_sum"  class="play_sum layer_box pa_10" title="推荐" popbox-action="<?php echo url('video/edit_play_sum'); ?>" popbox-get-data="<?php echo url('video/edit_play_sum'); ?>" popbox-area="560px,450px">
        播放数: <input class="base_text_s" name="play_sum"  value=""/>
         <input type="hidden" name="id"  value=""/>
        <div class="base_button sub_btn">保存</div>
