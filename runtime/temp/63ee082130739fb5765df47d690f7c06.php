        ...
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <a href="<?php echo url('recommend_content/'.$vo['type'],array('rec_id'=>$vo['id'])); ?>"><?php echo htmlentities($vo['count']); ?></a>
                        </td>
                        <td><?php echo enum_attr('recommend_space_types',$vo['type'],'name'); ?></td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:recommend_space:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('recommend_space/change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td><?php echo htmlentities(time_format($vo['create_time'])); ?></td>
                        <td>
                            <?php if(check_auth('admin:recommend_space:update',AUTH_UID)): ?>
