    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('admin:packages:add',AUTH_UID)): ?>
                    <li><a href="<?php echo url('add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>"
                           class="base_button base_button_s">新增</a></li>
                <?php endif; if(check_auth('admin:packages:delete',AUTH_UID)): ?>
            ...
            <div class="content_toolbar_search">
                <div class="base_group">
                    <div class="modal_select modal_select_s">
                        ...
                        <input name="status" type="hidden" class="modal_select_value finder" value="<?php echo input('status'); ?>"/>
                    ...
                    <div class="modal_select modal_select_s">
                        <span class="modal_select_text"></span>
                        <input name="channel" type="hidden" class="modal_select_value finder" value="<?php echo input('channel'); ?>"/>
                        <ul class="modal_select_list">
                            <li class="modal_select_option" value="">全部渠道</li>
                            <?php $_result=enum_array('packages_channel');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?>
                                <li class="modal_select_option" value="<?php echo htmlentities($channel['value']); ?>"><?php echo htmlentities($channel['name']); ?></li>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
                    <div class="modal_select modal_select_s">
                        <span class="modal_select_text"></span>
                        <input name="os" type="hidden" class="modal_select_value finder" value="<?php echo input('os'); ?>"/>
                        <ul class="modal_select_list">
                            <li class="modal_select_option" value="">全部平台</li>
                            <?php $_result=enum_array('packages_os');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$os): $mod = ($i % 2 );++$i;?>
                                <li class="modal_select_option" value="<?php echo htmlentities($os['value']); ?>"><?php echo htmlentities($os['name']); ?></li>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
                    <div class="modal_select modal_select_s">
                        <span class="modal_select_text"></span>
                        <input name="update_type" type="hidden" class="modal_select_value finder" value="<?php echo input('update_type'); ?>"/>
                        <ul class="modal_select_list">
                            <li class="modal_select_option" value="">全部类型</li>
                            <?php $_result=enum_array('packages_update_types');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$update_type): $mod = ($i % 2 );++$i;?>
                                <li class="modal_select_option" value="<?php echo htmlentities($update_type['value']); ?>"><?php echo htmlentities($update_type['name']); ?></li>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
                    <input name="code" class="base_text base_text_s finder" style="width:180px;" value="<?php echo input('code'); ?>" placeholder="内部版本号"/>
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;" value="<?php echo input('keyword'); ?>" placeholder="搜索ID、包名"/>
                    ...
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            ...
                            <?php if(!(empty($vo['url']) || (($vo['url'] instanceof \think\Collection || $vo['url'] instanceof \think\Paginator ) && $vo['url']->isEmpty()))): ?>
                                <a title="点击跳转到第三方地址" target="_blank" href="<?php echo htmlentities($vo['url']); ?>">第三方地址</a>
                            <?php endif; if(!(empty($vo['file_path']) || (($vo['file_path'] instanceof \think\Collection || $vo['file_path'] instanceof \think\Paginator ) && $vo['file_path']->isEmpty()))): ?>
                                <a title="点击下载安装包" class="fc_green" target="_blank" href="<?php echo htmlentities($vo['file_path']); ?>">安装包</a>
                            <?php endif; ?>
                        </td>
                        <td><?php echo htmlentities(enum_name($vo['channel'],'packages_channel')); ?></td>
                        <td>
                            <b>[ <?php echo htmlentities($vo['code']); ?>]</b><br/>
                            <?php echo htmlentities($vo['version']); ?>
                        </td>
                        <td><?php echo htmlentities(enum_name($vo['os'],'packages_os')); ?></td>
                        <td><?php echo htmlentities($vo['download_num']); ?></td>
                        <td><?php echo htmlentities($vo['filesize_str']); ?></td>
                        <td><?php echo htmlentities(enum_name($vo['update_type'],'packages_update_types')); ?></td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:packages:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0"
                                 tgradio-value="<?php echo htmlentities($vo['min_version']); ?>" tgradio-name="min_version"
                                 tgradio-on-name="启用" tgradio-off-name="普通"
                                 tgradio="<?php echo url('change_min_version',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:packages:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0"
                        ...
                        <td>
                            <?php if(check_auth('admin:packages:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><br/>
                            <?php endif; if(check_auth('admin:packages:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('del',array('id'=>$vo['id'])); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
