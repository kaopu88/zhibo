...
<head lang="en">
    ...
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/promoter/index.js?v=<?php echo config('upload.resource_version'); ?>"></script>
...
<div class="main">
  ...
  <div class="sub_catalog"></div>
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <div class="filter_search">
                        <input placeholder="<?php echo config('app.agent_setting.promoter_name'); ?>ID、手机号、昵称" type="text" name="keyword" value="<?php echo input('keyword'); ?>"/>
        ...
        <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['user_id']); ?>">
                        ...
                        <td>
                            <div class="thumb">
    <a href="<?php echo url('promoter/detail',['user_id'=>$vo['user_id']]); ?>" class="thumb_img thumb_img_avatar">
    ...
    <p class="thumb_info">
        <a href="<?php echo url('promoter/detail',['user_id'=>$vo['user_id']]); ?>">
            ...
            <?php echo htmlentities((str_hide($vo['phone'],3,4) ?: '未绑定')); ?>
                        ...
                        <td><?php echo htmlentities($vo['total_cons']); ?></td>
                        <td><?php echo htmlentities($vo['total_fans']); ?></td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:user:change_status')?'0':'1'; ?>" tgradio-on="1"
                                 tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>"
                                 tgradio-name="status"
                                 tgradio="<?php echo url('user/change_status',['id'=>$vo['user_id']]); ?>"></div>
                        </td>
                        <td>
                            <?php echo htmlentities($vo['client_num']); ?>&nbsp;<a ajax="post" title="校正客户数量"
                                                     href="<?php echo url('correct_client_num',['user_id'=>$vo['user_id']]); ?>"><span
    ...
    <?php if($vo['agent_num'] > '0'): if(!(empty($vo['agent_list']) || (($vo['agent_list'] instanceof \think\Collection || $vo['agent_list'] instanceof \think\Paginator ) && $vo['agent_list']->isEmpty()))): if(is_array($vo['agent_list']) || $vo['agent_list'] instanceof \think\Collection || $vo['agent_list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['agent_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fo): $mod = ($i % 2 );++$i;?>
        ...
        <?php endif; endif; ?>
                        ...
                        <td>
                            <a href="<?php echo url('promoter/detail',['user_id'=>$vo['user_id']]); ?>"><?php echo config('app.agent_setting.promoter_name'); ?>详情</a><br/>
                            <?php if(check_auth('admin:user:assign_agent',AUTH_UID)): ?>
                                <a data-transfer="user" class="transfer_btn" data-rel="promoter"
                                   data-id="<?php echo htmlentities($vo['user_id']); ?>" href="javascript:;">移交客户</a><br/>
                            <?php endif; if(check_auth('admin:promoter:cancel',AUTH_UID)): ?>
                                <a ajax="get" ajax-confirm="是否确认取消<?php echo config('app.agent_setting.promoter_name'); ?>？" class="fc_red"
                                   href="<?php echo url('cancel',['user_id'=>$vo['user_id']]); ?>">取消<?php echo config('app.agent_setting.promoter_name'); ?></a>
                            <?php endif; ?>
                        </td>
                    </tr>
