    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('taokeshop:anchor_goods_cate:add',AUTH_UID)): ?>
                    <li><a href="<?php echo url('add',['pcat_id'=>input('pcat_id'),'cat_id'=>input('cat_id')]); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a></li>
                <?php endif; if(check_auth('taokeshop:anchor_goods_cate:delete',AUTH_UID)): ?>
            ...
            <div class="content_toolbar_search">
                <div class="base_group">
                    ...
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;" value="<?php echo input('keyword'); ?>" placeholder="搜索商品ID、标题"/>
        ...
        <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['cate_id']); ?>">
                        <td><input class="list_id" type="checkbox" name="ids[]" value="<?php echo htmlentities($vo['cate_id']); ?>"/></td>
                        ...
                        <td>
                            <div tgradio-not="<?php echo check_auth('taoke:anchor_goods_cate:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0"
                                 tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('change_status',array('cate_id'=>$vo['cate_id'])); ?>"></div>
                        </td>
                        <td>
                            <?php echo htmlentities(time_format($vo['create_time'],'','Y-m-d H:i:s')); ?>
                        </td>
                        <td>
                            <?php if(check_auth('taokeshop:anchor_goods_cate:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',array('cate_id'=>$vo['cate_id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><br/>
                            <?php endif; if(check_auth('taokeshop:anchor_goods_cate:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('del',array('cate_id'=>$vo['cate_id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
