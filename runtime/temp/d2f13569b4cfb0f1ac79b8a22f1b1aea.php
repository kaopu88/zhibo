            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <div style="float: left">
                        <?php if(check_auth('admin:topic:add',AUTH_UID)): ?>
                            <a class="base_button base_button_s" href="<?php echo url('add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">新增类型</a>
                        <?php endif; ?>
                    </div>

                    <div class="filter_search">
                        <input placeholder="名称" type="text" name="keyword" value="<?php echo input('keyword'); ?>"/>
        ...
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td><?php echo htmlentities($vo['default_aid']); ?></td>
                        <td><?php echo htmlentities(time_format($vo['create_time'],'无','date')); ?></td>
                        <td>
                            <?php if(check_auth('admin:topic:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',['id'=>$vo['id']]); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><br/>
                            <?php endif; if(check_auth('admin:topic:delete',AUTH_UID)): ?>
