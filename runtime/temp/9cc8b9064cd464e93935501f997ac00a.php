...
<head lang="en">
    ...
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/anchor_exp_level/index.js?v=<?php echo config('upload.resource_version'); ?>"></script>
  ...
  <div class="sub_catalog"></div>
    ...
    <div class="pa_20">
        ...
        <div class="filter_box mt_10">
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <div style="float: left">
                        <?php if(check_auth('admin:anchor_exp_level:add',AUTH_UID)): ?>
                            <a href="<?php echo url('add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a>
                        <?php endif; if(check_auth('admin:anchor_exp_level:delete',AUTH_UID)): ?>
            ...
            <input type="hidden" name="is_recommend" value="<?php echo htmlentities($get['is_recommend']); ?>"/>
        </div>
        ...
        <table class="content_list mt_10 sm_width">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['levelid']); ?>">
                        <td><input class="list_id" type="checkbox" name="ids[]" value="<?php echo htmlentities($vo['levelid']); ?>"/></td>
                        <td><?php echo htmlentities($vo['levelid']); ?></td>
                        <td>
                            <div class="thumb">
                                <a href="javascript:;"
                                   class="thumb_img thumb_img_avatar">
                                    <img src="<?php echo img_url($vo['icon'],'200_200','avatar'); ?>"/>
                                </a>
                                <p class="thumb_info">
                                    <a href="javascript:;">
                                        <?php echo htmlentities($vo['levelname']); ?>
                                    </a>
                                </p>
                            </div>
                        </td>
                        <td><?php echo htmlentities($vo['level_up']); ?></td>
                        <td>
                            <?php echo htmlentities(time_format($vo['addtime'],'无','date')); ?>
                        </td>
                        <td>
                            <?php if(check_auth('admin:anchor_exp_level:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',array('id'=>$vo['levelid'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a>
                            <?php endif; if(check_auth('admin:anchor_exp_level:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('delete',array('id'=>$vo['levelid'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
