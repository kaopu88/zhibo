...
<head lang="en">
    ...
    <script>
        ...
        var myConfig = {
            action:'<?php echo url("admin/user/index"); ?>',
            list: [
                {
                    name: 'status',
                    title: '启用状态',
                ...
                },
                {
                    name: 'level',
                    title: '用户等级',
                    get: '<?php echo url("common/get_levels"); ?>'
                },
                {
                    name: 'province',
                    title: '所在省份',
        ...
        <div class="filter_box mt_10">
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <?php if(check_auth('admin:user:add',AUTH_UID)): ?>
                        <div layer-area="600px,450px" layer-open="<?php echo url('user/reg'); ?>"
                             class="base_button base_button_s reg_btn">注册新用户
                        </div>
                    <?php endif; if(check_auth('admin:user:assign_agent',AUTH_UID)): ?>
                        <div data-listids="1" data-transfer="user"
                             class="base_button base_button_s reg_btn transfer_btn">批量分配<?php echo config('app.agent_setting.agent_name'); ?>
                        </div>
                    <?php endif; if(check_auth('admin:user:change_status',AUTH_UID)): ?>
                        <div ajax="post" ajax-url="<?php echo url('user/change_status',['status'=>'1']); ?>" ajax-target="list_id"
                             class="base_button base_button_s base_button_gray">启用
                        </div>
                        <div ajax-before="tgradioStatusBefore2" ajax="post"
                             ajax-url="<?php echo url('user/change_status',['status'=>'0']); ?>" ajax-target="list_id"
                             class="base_button base_button_s base_button_gray">禁用
                        </div>
                    <?php endif; ?>
                    <div class="filter_search">
                        <input autocomplete="0" placeholder="用户ID、手机号、昵称" type="text" name="keyword" value="<?php echo input('keyword'); ?>"/>
        ...
        <div class="table_slide">
            <table class="content_list mt_10 table_fixed">
                ...
                <tr>
                    ...
                    <?php if($distribute_status == '1'): ?> <td style="width: 10%;">分销等级</td><?php endif; ?>
                    <td style="width: 10%;">用户属性</td>
                    <td style="width: 10%;"><?php echo APP_BEAN_NAME; ?></td>
            ...
            <br/><?php echo config('app.agent_setting.agent_name'); ?>：<?php echo htmlentities($vo['agent_name']); endif; ?>
                            ...
                            <td>
                                <?php if(empty($vo['pid'])): ?>
                                    暂无
                                    <?php else: ?>
                                        <a href="<?php echo url('user/detail',['user_id'=>$vo['pid']]); ?>" > <?php echo htmlentities($vo['pid']); ?> </a>
                                <?php endif; ?>
...
<?php endif; ?>
                            ...
                            <?php if($distribute_status == '1'): ?>
                                <td>
                                    <?php echo htmlentities((isset($vo['fenxiao_level_name']['level_name']) && ($vo['fenxiao_level_name']['level_name'] !== '')?$vo['fenxiao_level_name']['level_name']:'默认')); ?>
                                </td>
                            <?php endif; ?>
                            <td>
...
<?php endif; if(check_auth('admin:user:change_contact_status',AUTH_UID)): ?>
    ...
    私信：<?php switch($vo['contact_status']): case "0": ?><span class="fc_red">关闭</span><?php break; case "1": ?><span class="fc_green">开启</span><?php break; endswitch; endif; ?>
                            ...
                            <td>
                                <a href="<?php echo url('user/detail',['user_id'=>$vo['user_id']]); ?>">用户详情</a>
                                <?php if(check_auth('admin:recommend_content:rec_user',AUTH_UID)): ?>
                                    <br/><a poplink="recommend_box" data-query="id=<?php echo htmlentities($vo['user_id']); ?>&type=user" href="javascript:;">推荐(<?php echo htmlentities($vo['rec_num']); ?>)</a>
                                <?php endif; if(check_auth('admin:user:assign_agent',AUTH_UID)): ?>
                                    <br/><a class="transfer_btn" data-transfer="user" data-id="<?php echo htmlentities($vo['user_id']); ?>"
                                            href="javascript:;">分配<?php echo config('app.agent_setting.agent_name'); ?></a>
                                <?php endif; if(check_auth('admin:user:roler',AUTH_UID)): ?>
                                    <br/><a layer-open="<?php echo url('user/agent_list',['user_id'=>$vo['user_id']]); ?>"
