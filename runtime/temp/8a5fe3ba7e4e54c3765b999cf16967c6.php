    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('admin:admin_group:add',AUTH_UID)): ?>
                    <li><a href="<?php echo url('admin_group/add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a> </li>
                <?php endif; if(check_auth('admin:admin_group:delete',AUTH_UID)): ?>
                    <li><a href="<?php echo url('admin_group/del'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a></li>
                <?php endif; if(check_auth('admin:setting:data_update',AUTH_UID)): ?>
                    <li style="line-height: 30px;font-size: 12px;padding-left: 10px;">
                        <a data-to="admin_group:<?php echo htmlentities($data_v); ?>" class="update_data_version" href="javascript:;"><?php echo $data_v=='last'?'已更新至最新版本':'更新至:'.$data_v; ?></a>
                    </li>
                <?php endif; ?>
            </ul>
            <div class="content_toolbar_search">
                <div class="base_group">
                    ...
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;" value="<?php echo input('keyword'); ?>" placeholder="搜索ID、名称" />
        ...
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        <td style="width: 5%;"><input class="list_id" type="checkbox" name="ids[]" value="<?php echo htmlentities($vo['id']); ?>" /></td>
                        <td style="width: 10%;"><?php echo htmlentities($vo['id']); ?></td>
                        <td style="width: 12%;"><?php echo htmlentities($vo['name']); ?></td>
                        <td style="width: 17%;"><?php echo htmlentities($vo['descr']); ?></td>
                        <td style="width: 8%;"><?php echo htmlentities($vo['rules_num']); ?></td>
                        <td style="width: 8%;"><?php echo htmlentities($vo['works_num']); ?></td>
                        <td style="width: 10%;"><?php echo htmlentities($vo['num']); ?></td>
                        <td style="width: 10%;">
                            <div tgradio-not="<?php echo check_auth('admin:admin_group:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('admin_group/change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td style="width: 10%;"><?php echo htmlentities(time_format($vo['create_time'])); ?></td>
                        <td style="width: 10%;">
                            <?php if(check_auth('admin:admin_group:update',AUTH_UID)): ?><a href="<?php echo url('admin_group/edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><?php endif; if(check_auth('admin:admin_group:delete',AUTH_UID)): ?> <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('admin_group/del',array('id'=>$vo['id'])); ?>">删除</a><?php endif; ?>
                        </td>
                    </tr>
