...
<head lang="en">
        ...
        var myConfig = {
            list: [
                ...
                },
                {
                    ...
                    auto_sub: false,
                    get: '<?php echo url("common/get_category"); ?>'
                },
                {
                    name: 'cat_id',
                    parent: 'pcat_id',
    ...
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/resources/index.js?v=<?php echo config('upload.resource_version'); ?>"></script>
  ...
  <div class="sub_catalog"></div>
    ...
    <div class="pa_20">
        ...
        <div class="filter_box mt_10">
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    ...
                    <div class="filter_search">
                        <input placeholder="ID、标题、资源名称" type="text" name="keyword" value="<?php echo input('keyword'); ?>"/>
            ...
            <input type="hidden" name="pcat_id" value="<?php echo htmlentities($get['pcat_id']); ?>"/>
            <input type="hidden" name="cat_id" value="<?php echo htmlentities($get['cat_id']); ?>"/>
            <input type="hidden" name="hot" value="<?php echo htmlentities($get['hot']); ?>"/>
            <input type="hidden" name="new" value="<?php echo htmlentities($get['new']); ?>"/>
        </div>
        ...
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr>
                        ...
                        <td><?php echo htmlentities($vo['file_url']); ?></td>
                        <td><?php echo htmlentities($vo['file_size']); ?></td>
                        <td><div tgradio-not="<?php echo check_auth('admin:resources:update')?'0':'1'; ?>" tgradio-on="1"
                                 tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['hot']); ?>"
                                 tgradio-on-name="热门" tgradio-off-name="普通"
                                 tgradio-name="hot"
                                 tgradio="<?php echo url('resources/change_hot_status',['id'=>$vo['id']]); ?>"></div></td>
                        <td><div tgradio-not="<?php echo check_auth('admin:resources:update')?'0':'1'; ?>" tgradio-on="1"
                                 tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['new']); ?>"
                                 tgradio-on-name="最新" tgradio-off-name="普通"
                                 tgradio-name="new"
                                 tgradio="<?php echo url('resources/change_new_status',['id'=>$vo['id']]); ?>"></div></td>
                        <td><?php echo htmlentities($vo['sort']); ?></td>
                        <td>
                        <div tgradio-not="<?php echo check_auth('admin:resources:update')?'0':'1'; ?>" tgradio-on="1"
                             tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>"
                             tgradio-name="status"
                             tgradio="<?php echo url('resources/change_status',['id'=>$vo['id']]); ?>">
                        </div>
                        </td>
                        <td>
                        ...
                        <td>
                            <?php if(check_auth('admin:resources:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('resources/del',array('id'=>$vo['id'])); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
