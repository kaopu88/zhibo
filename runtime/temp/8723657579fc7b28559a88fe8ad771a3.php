                    ...
                    <div class="modal_select modal_select_s">
                        ...
                        <ul class="modal_select_list">
                            ...
                            <?php if(is_array($cat_list) || $cat_list instanceof \think\Collection || $cat_list instanceof \think\Paginator): $i = 0; $__LIST__ = $cat_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat): $mod = ($i % 2 );++$i;?>
                                <li class="modal_select_option" value="<?php echo htmlentities($cat['id']); ?>"><?php echo htmlentities($cat['name']); ?></li>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
                    <?php if(!(empty($cat_list2) || (($cat_list2 instanceof \think\Collection || $cat_list2 instanceof \think\Paginator ) && $cat_list2->isEmpty()))): ?>
                        <div class="modal_select modal_select_s">
                            <span class="modal_select_text"></span>
                            <input name="cat_id" type="hidden" class="modal_select_value finder"
                                   value="<?php echo input('cat_id'); ?>"/>
                            <ul class="modal_select_list">
                                <li class="modal_select_option" value="">二级类目（全部）</li>
                                <?php if(is_array($cat_list2) || $cat_list2 instanceof \think\Collection || $cat_list2 instanceof \think\Paginator): $i = 0; $__LIST__ = $cat_list2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat2): $mod = ($i % 2 );++$i;?>
                                    <li class="modal_select_option" value="<?php echo htmlentities($cat2['id']); ?>"><?php echo htmlentities($cat2['name']); ?></li>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </ul>
                        </div>
                    <?php endif; ?>
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;"
                           value="<?php echo input('keyword'); ?>" placeholder="搜索ID、标题"/>
        ...
        <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <?php echo htmlentities($vo['id']); if(!(empty($vo['mark']) || (($vo['mark'] instanceof \think\Collection || $vo['mark'] instanceof \think\Paginator ) && $vo['mark']->isEmpty()))): ?>
                                <br/><?php echo htmlentities($vo['mark']); endif; ?>
                        </td>
                        <td>
                            <div class="thumb">
                                <a href="<?php echo url('edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="thumb_img">
                                    <img src="<?php echo img_url($vo['image'],'200_200','thumb'); ?>"/>
                                </a>
                                <p class="thumb_info">
                        ...
                        <td>
                            <span><?php echo htmlentities($vo['pcat_info']['name']); ?></span><br/>
                            <span><?php echo htmlentities($vo['cat_info']['name']); ?></span>
                        </td>
                        <td>admin</td>
                        <td><?php echo htmlentities($vo['sort']); ?></td>
                        <td>
                            点击量: <?php echo htmlentities($vo['pv']); ?><br/>
                            评论量:
                            <?php if(check_auth('admin:article_comment:select',AUTH_UID)): ?>
                                <a target="_blank" href="<?php echo url('article_comment/index',['rel_type'=>'art','rel_id'=>$vo['id']]); ?>"><?php echo htmlentities($vo['comment_num']); ?></a>
                                <?php else: ?>
                                <?php echo htmlentities($vo['comment_num']); endif; ?>
                            <br/>
                            点赞量: <?php echo htmlentities($vo['like_num']); ?>
                        </td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:article:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0"
                                 tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status"
                                 tgradio="<?php echo url('change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td>
                            <?php echo htmlentities(time_format($vo['create_time'])); ?><br/>
                            <?php echo htmlentities(time_format($vo['release_time'])); ?>
                        </td>
                        <td>
                            <?php if(check_auth('admin:article:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑文章</a><br/>
                            <?php endif; if(check_auth('admin:recommend_content:rec_art',AUTH_UID)): ?>
                                <a poplink="recommend_box" data-query="id=<?php echo htmlentities($vo['id']); ?>&type=art" href="javascript:;">推荐(<?php echo htmlentities($vo['rec_num']); ?>)</a><br/>
                            <?php endif; if(check_auth('admin:article:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('del',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除文章</a>
                            <?php endif; ?>
                        </td>
                    </tr>
