  ...
  <div class="sub_catalog"></div>
    ...
    <div class="pa_20">
        ...
        <div class="filter_box mt_10">
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <div style="float: left">
                        <?php if(check_auth('admin:live_channel:add',AUTH_UID)): ?>
                            <a class="base_button base_button_s" href="<?php echo url('live_channel/add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">新增频道</a>
                        <?php endif; if(check_auth('admin:live_channel:delete',AUTH_UID)): ?>
                    ...
                    <div class="filter_search">
                        <input placeholder="父频道ID" type="text" name="parent_id" value="<?php echo input('parent_id'); ?>"/>
            ...
            <input type="hidden" name="sub_channel" value="<?php echo input('sub_channel'); ?>"/>
        </div>
         ...
         <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <div class="thumb">
                                <a rel="thumb" href="<?php echo img_url($vo['icon'],'','thumb'); ?>" class="thumb thumb_img user_base_avatar fancybox" alt="">
                        ...
                        <td><?php echo htmlentities($vo['parent_str']); ?></td>
                        <td><?php echo !empty($vo['sub_channel']) ? '是'  :  '否'; ?></td>
                        <td><?php echo htmlentities($vo['description']); ?></td>
                        <td><?php echo htmlentities($vo['sort_order']); ?></td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:live_channel:update')?'0':'1'; ?>"
                                 tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status"
                                 tgradio="<?php echo url('change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td>
                            <?php if(check_auth('admin:live_channel:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',['id'=>$vo['id']]); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑频道</a><br/>
                            <?php endif; if(check_auth('admin:live_channel:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get"
                                   href="<?php echo url('delete',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除频道</a>
                            <?php endif; ?>
                        </td>
                    </tr>
