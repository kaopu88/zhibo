...
<head lang="en">
    ...
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/loss/index.js?v=<?php echo config('upload.resource_version'); ?>"></script>
    ...
    layui.use(['element', 'layer'], function(){
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    ...
                    <div class="filter_search">
                        <input placeholder="<?php echo config('app.agent_setting.agent_name'); ?>ID" type="text" name="agent_id" value="<?php echo input('agent_id'); ?>"/>
        ...
        <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr class="<?php echo $vo['id']=='1' ? 'id' : ''; ?>" data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>订单号：<?php echo htmlentities($vo['descr']); ?></td>
                        <td><?php echo htmlentities($vo['millet']); ?></td>
                        <td><?php echo htmlentities($vo['rmb']); ?></td>
                        <td><?php echo htmlentities($vo['cash_fee']); ?></td>
                        <td><?php echo htmlentities($vo['cash_taxes']); ?></td>
                        <td>
                            <?php switch($vo['casy_type']): case "0": ?>
                                ...
                                <?php break; endswitch; ?>
                            姓名:<?php echo htmlentities($vo['name']); ?><br/>
                            联系方式:<?php echo htmlentities($vo['contact_phone']); ?><br/>
                            账户:<?php echo htmlentities($vo['account']); ?><br/>
                            <?php if($vo['casy_type'] == '2'): ?>
                                开户行:<?php echo htmlentities($vo['card_name']); ?><br/>
                            <?php endif; ?>
                        </td>

                        <td>
                            <?php switch($vo['audit_status']): case "2": ?>
                                    <span class="fc_red">已拒绝 (原因：<?php echo htmlentities($vo['admin_remark']); ?>)</span>
                                <?php break; case "1": ?>
                        ...
                        <td>
                            ...
                            <?php if(!(empty($vo['handler_time']) || (($vo['handler_time'] instanceof \think\Collection || $vo['handler_time'] instanceof \think\Paginator ) && $vo['handler_time']->isEmpty()))): ?>处理：<?php echo htmlentities(time_format($vo['handler_time'],'未处理')); endif; ?>
                        </td>
                        <td>
...
<div title="提现编辑" class="layer_box millet_audit_update pa_10" dom-key="millet_audit_update"
     popbox-action="<?php echo url('agent_withdrawal/change_status'); ?>" popbox-get-data="<?php echo url('agent_withdrawal/change_status'); ?>" popbox-area="700px,550px" onsubmit="return sumbit_sure()">
    <div class="table_slide">
        <table class="content_info2 xs_width">
            <tr class="edit_tr">
                <td class="field_name">标注</td>
                <td>
                    <textarea name="admin_remark" style="height: 70px;" class="admin_remark"></textarea>
                </td>
            </tr>
            <tr class="edit_tr">
