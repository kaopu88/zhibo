        ...
        var myConfig = {
            list: [
                ...
                },
                {
                    name: 'space_id',
                    title: '广告位',
                    get: '<?php echo url("ad_space/get_spaces"); ?>'
                },
                {
                    name: 'os',
                    title: '投放平台',
                    opts: JSON.parse('<?php echo json_encode(enum_array("ad_os")); ?>')
                }
            ]
        };
    </script>
        ...
        <div class="filter_box mt_10">
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <?php if(check_auth('admin:ad_content:add',AUTH_UID)): ?>
                        <a href="<?php echo url('admin/ad_content/add',['space_id'=>$_info['space_id']]); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a>
                    <?php endif; if(check_auth('admin:ad_content:add',AUTH_UID)): ?>
                        <a href="<?php echo url('admin/ad_content/del'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a>
                    <?php endif; ?>
            ...
            <input type="hidden" name="space_id" value="<?php echo input('space_id'); ?>"/>
            <input type="hidden" name="os" value="<?php echo input('os'); ?>"/>
        </div>
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <a href="<?php echo url('admin/ad_space/edit',['id'=>$vo['space_info']['id']]); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>"><?php echo htmlentities($vo['space_info']['name']); ?></a>
                        </td>
                        <td>
                            <div class="thumb">
                                <a title="<?php echo htmlentities($vo['title']); ?>" rel="common"
                                   href="<?php echo img_url($vo['image']['common'],'','thumb2'); ?>" class="thumb_img fancybox">
                                    <img src="<?php echo img_url($vo['image']['common'],'200_200','thumb2'); ?>"/>
                                </a>
                                <p class="thumb_info">
                                    <a href="<?php echo url('edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">
                                        <?php echo htmlentities($vo['title']); if(!(empty($vo['url']) || (($vo['url'] instanceof \think\Collection || $vo['url'] instanceof \think\Paginator ) && $vo['url']->isEmpty()))): ?>
                                            <a title="<?php echo htmlentities($vo['url']); ?>" target="_blank" href="<?php echo htmlentities($vo['url']); ?>"><span class="icon-link"></span></a>
                                        <?php endif; ?>
                                    </a>
                                </p>
                            </div>
                        ...
                        <td>
                            <?php if(empty($vo['os']) || (($vo['os'] instanceof \think\Collection || $vo['os'] instanceof \think\Paginator ) && $vo['os']->isEmpty())): ?>
                                <span>全部</span>
                                <?php else: if(is_array($vo['os']) || $vo['os'] instanceof \think\Collection || $vo['os'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['os'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$os_item): $mod = ($i % 2 );++$i;?>
                                    <span><?php echo enum_attr('ad_os',$os_item,'name'); ?></span>
                                <?php endforeach; endif; else: echo "" ;endif; endif; ?>
                            <br/>
                            <?php echo htmlentities($vo['code_min']); ?>-<?php echo htmlentities($vo['code_max']); ?>
                        </td>
                        <td>
                            <?php switch($vo['display_status']): case "0": ?>
                                ...
                                <?php break; endswitch; ?>
                            <br/>
                            <?php echo htmlentities(time_format($vo['start_time'])); ?><br/>
                            <?php echo htmlentities(time_format($vo['end_time'])); ?>
                        </td>
                        <td>
                            <?php if(empty($vo['purview']) || (($vo['purview'] instanceof \think\Collection || $vo['purview'] instanceof \think\Paginator ) && $vo['purview']->isEmpty())): ?>
                                <span>全部可见</span>
                                <?php else: if(is_array($vo['purview']) || $vo['purview'] instanceof \think\Collection || $vo['purview'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['purview'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$purview_item): $mod = ($i % 2 );++$i;?>
                                    <span><?php echo enum_attr('ad_purviews',$purview_item,'name'); ?></span>
                                <?php endforeach; endif; else: echo "" ;endif; endif; ?>
                        </td>
                        <td>
                            排序值:<?php echo htmlentities($vo['sort']); ?><br/>
                            浏览量:<?php echo htmlentities($vo['pv']); ?><br/>
                            点击量:<?php echo htmlentities($vo['click']); ?>
                        </td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:ad_content:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('ad_content/change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td><?php echo htmlentities(time_format($vo['create_time'],'','date')); ?></td>
                        <td>
                            <?php if(check_auth('admin:ad_content:update',AUTH_UID)): ?>
                                <a href="<?php echo url('ad_content/edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><br/>
                            <?php endif; if(check_auth('admin:ad_content:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('ad_content/del',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
