...
<head lang="en">
    ...
    <link rel="stylesheet" href="/static/vendor/jsTree/themes/bugu/style.css?v=<?php echo config('upload.resource_version'); ?>" />
    <style>
        .rule_form{
            display: none;
            padding: 15px;
    ...
    <script src="/static/vendor/jsTree/jstree.min.js?v=<?php echo config('upload.resource_version'); ?>"></script>
    <script src="/static/admin/js/tree_table.js?v=<?php echo config('upload.resource_version'); ?>"></script>
    <script>
        var config={
            item_name:'规则',
            parent_key:'cid',
            get_tree:'<?php echo url("admin_rule/get_tree"); ?>',
            del_url:'<?php echo url("admin_rule/del"); ?>',
            add_url:'<?php echo url("admin_rule/add"); ?>',
            edit_url:'<?php echo url("admin_rule/edit"); ?>',
            sort_url:'<?php echo url("admin_rule/sort"); ?>',
    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns rule_btns">
                ...
                <?php if(check_auth('admin:admin_rule:delete',AUTH_UID)): ?>
                ...
                <?php endif; if(check_auth('admin:setting:data_update',AUTH_UID)): ?>
                    <li style="line-height: 30px;font-size: 12px;padding-left: 10px;">
                        <a data-to="admin_rule:<?php echo htmlentities($data_v); ?>" class="update_data_version" href="javascript:;"><?php echo $data_v=='last'?'已更新至最新版本':'更新至:'.$data_v; ?></a>
                    </li>
                <?php endif; ?>
            </ul>
