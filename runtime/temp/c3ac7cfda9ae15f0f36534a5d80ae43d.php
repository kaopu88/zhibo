  ...
  <div class="sub_catalog"></div>
    ...
    <div class="pa_20">
        ...
        <div class="filter_box mt_10">
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <div style="float: left">
                        <?php if(check_auth('admin:gift:add',AUTH_UID)): ?>
                            <a class="base_button base_button_s" href="<?php echo url('gift/add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">新增礼物</a>
                        <?php endif; if(check_auth('admin:gift:delete',AUTH_UID)): ?>
            ...
            <input type="hidden" name="cid" value="<?php echo input('cid'); ?>"/>
        </div>
        ...
        <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <div class="thumb">
                                <a rel="thumb" href="<?php echo img_url($vo['picture_url'],'','thumb'); ?>" class="thumb thumb_img user_base_avatar fancybox" alt="">
                                    <img src="<?php echo img_url($vo['picture_url'],'200_200','avatar'); ?>"/>
                                </a>
                                <p class="thumb_info">
                                    <a href="javascript:;">
                        ...
                        <td>
                            单价：<?php echo htmlentities($vo['price']); ?><br/>
                            折扣：<?php echo htmlentities($vo['discount_str']); ?><br/>
                            等值<?php echo config('app.product_info.bean_name'); ?>：<?php echo htmlentities($vo['conv_millet']); ?><br/>
                            总销量：<?php echo htmlentities($vo['sales']); ?><br/>
                            性质：<?php echo htmlentities($vo['type_str']); ?><br/>
                            点击提示信息：<?php echo htmlentities($vo['tips']); ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:gift:update')?'0':'1'; ?>" tgradio-on="1"
                                 tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['isguard']); ?>"
                                 tgradio-name="isguard" tgradio-on-name="是" tgradio-off-name="否"
                                 tgradio="<?php echo url('change_guard',['id'=>$vo['id']]); ?>"></div>
                        </td>
                        <td><?php echo htmlentities($vo['sort']); ?></td>
                        <td><?php echo htmlentities($vo['show_params']); ?></td>
                        <td><?php echo htmlentities($vo['privileges_str']); ?></td>
                        <td><?php echo htmlentities(time_format($vo['create_time'],'无','date')); ?></td>
                        <td>
                            <?php if(check_auth('admin:gift:update',AUTH_UID)): ?>
                                <a data-id="gift_id:<?php echo htmlentities($vo['id']); ?>" poplink="badge_box" href="javascript:;">
                                    礼物角标
                                </a><br/>
                            <?php endif; if(check_auth('admin:gift:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',['id'=>$vo['id']]); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑礼物</a><br/>
                            <?php endif; if(check_auth('admin:gift:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('delete',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除礼物</a>
                            <?php endif; ?>
                        </td>
                    </tr>
...
<div dom-key="badge_box" class="layer_box badge_box pa_10" title="礼物角标" popbox-action="<?php echo url('gift/update_badge'); ?>"
     popbox-area="520px,400px" popbox-get-data="<?php echo url('gift/update_badge'); ?>">
