...
<script>
    ...
    <p class="thumb_info">
        <a href="<?php echo url('user/detail',['user_id'=>$vo['user']['user_id']]); ?>">
            ...
            <?php echo htmlentities((str_hide($vo['user']['phone'],3,4) ?: '未绑定')); ?>
                                ...
                                <td>
                                    <?php echo htmlentities($vo['app_admin']['id']); ?><br/>
                                    <?php echo htmlentities(user_name($vo['app_admin'])); ?>
                                </td>
                                <td>
                                    人民币：<?php echo htmlentities($vo['total_fee']); ?><br/>
                                    <?php echo APP_BEAN_NAME; ?>：<?php echo htmlentities($vo['bean']); ?><br/>
                                    <?php echo htmlentities(number_2_rmb($vo['total_fee'])); ?>
                                </td>
                                <td><?php if($vo['pay_method'] == 'cash'): ?>
    [现金]
    <?php else: ?>
    [<?php echo htmlentities(enum_attr(recharge_pay_methods,$vo['pay_method'])); ?>]<br/>
    <?php if(!(empty($vo['pay_name']) || (($vo['pay_name'] instanceof \think\Collection || $vo['pay_name'] instanceof \think\Paginator ) && $vo['pay_name']->isEmpty()))): ?><?php echo htmlentities($vo['pay_name']); ?><br/><?php endif; if(!(empty($vo['pay_account']) || (($vo['pay_account'] instanceof \think\Collection || $vo['pay_account'] instanceof \think\Paginator ) && $vo['pay_account']->isEmpty()))): ?><?php echo htmlentities($vo['pay_account']); endif; endif; if(!(empty($vo['remark']) || (($vo['remark'] instanceof \think\Collection || $vo['remark'] instanceof \think\Paginator ) && $vo['remark']->isEmpty()))): ?><br/><span class="fc_gray">备注：<?php echo htmlentities($vo['remark']); ?></span><?php endif; ?></td>
                                <td><?php switch($vo['audit_status']): case "0": ?>
        <span class="fc_black">待审核</span>
    <?php break; case "1": ?>
    ...
    <?php break; endswitch; ?></td>
                                ...
                                <td>
                                    <?php switch($vo['audit_status']): case "0": if(check_auth('admin:recharge_app:audit',AUTH_UID)): ?>
                                                <a data-id="id:<?php echo htmlentities($vo['id']); ?>" poplink="recharge_app_handler"
                                                href="javascript:;">处理</a><br/>
                                            <?php endif; ?>
                                            <a data-query="id=<?php echo htmlentities($vo['id']); ?>&type=audit_recharge" poplink="task_transfer_box"
