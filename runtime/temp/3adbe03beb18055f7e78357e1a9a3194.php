    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('taokeshop:shop:delete',AUTH_UID)): ?>
                    <li><a href="<?php echo url('del'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a></li>
                <?php endif; ?>
            </ul>
            <div class="content_toolbar_search">
                <div class="base_group">
                    ...
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;" value="<?php echo input('keyword'); ?>" placeholder="搜索小店标题"/>
         ...
         <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <?php if(!(empty($vo['title']) || (($vo['title'] instanceof \think\Collection || $vo['title'] instanceof \think\Paginator ) && $vo['title']->isEmpty()))): ?>
                                <?php echo htmlentities($vo['title']); else: ?>
                                暂无
                            <?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="thumb">
                                <?php if(!(empty($vo['bg_img']) || (($vo['bg_img'] instanceof \think\Collection || $vo['bg_img'] instanceof \think\Paginator ) && $vo['bg_img']->isEmpty()))): ?>
                                    <a rel="thumb" href="<?php echo img_url($vo['bg_img'],'','thumb'); ?>" class="thumb thumb_img user_base_avatar fancybox" alt="">
                                        <img src="<?php echo img_url($vo['bg_img'],'200_200','thumb'); ?>"/>
                                    </a>
                                <?php endif; ?>
                            </div>
                        </td>
                        <td>
                            <div class="thumb">
                                <?php if(!(empty($vo['images']) || (($vo['images'] instanceof \think\Collection || $vo['images'] instanceof \think\Paginator ) && $vo['images']->isEmpty()))): if(is_array($vo['images']) || $vo['images'] instanceof \think\Collection || $vo['images'] instanceof \think\Paginator): $k = 0; $__LIST__ = $vo['images'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vm): $mod = ($k % 2 );++$k;?>
                                        <img src="<?php echo img_url($vm,'67_90','thumb'); ?>"/>
                                    <?php endforeach; endif; else: echo "" ;endif; endif; ?>
                            </div>
                        </td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('taokeshop:shop:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0"
                                 tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('changeStatus',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td>
                            <?php echo htmlentities(time_format($vo['create_time'],'','Y-m-d H:i:s')); ?>
                        </td>
                        <td>
                            <!--<?php if(check_auth('taokeshop:shop:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><br/>
                            <?php endif; ?>-->
                            <?php if(check_auth('taokeshop:shop:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('del',array('id'=>$vo['id'], "user_id"=>$vo['user_id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
