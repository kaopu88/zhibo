    ...
    layui.use(['element', 'layer'], function(){
        ...
        <div class="bg_container">
            <div class="content_toolbar mt_10">
                <?php if(input('audit_status') == '0'): if(check_auth('admin:user_data_deal:check',AUTH_UID)): ?>
                    <div ajax="post" ajax-url="<?php echo url('UserDataDeal/batch_deal',['audit_status'=>'1']); ?>"
                        ajax-target="list_id"
                    ...
                    <div class="base_group">
                        <div class="modal_select modal_select_s">
                            ...
                            <input name="audit_status" type="hidden" class="modal_select_value finder"
                                value="<?php echo input('audit_status'); ?>"/>
                        ...
                        <input name="keyword" class="base_text base_text_s finder" style="width:180px;"
                            value="<?php echo input('keyword'); ?>" placeholder="用户ID"/>
    ...
    <p class="thumb_info">
        <a href="<?php echo url('user/detail',['user_id'=>$vo['user']['user_id']]); ?>">
            ...
            <?php echo htmlentities((str_hide($vo['user']['phone'],3,4) ?: '未绑定')); ?>
                                ...
                                <td>
                                    <?php if(!(empty($vo['data']['avatar']) || (($vo['data']['avatar'] instanceof \think\Collection || $vo['data']['avatar'] instanceof \think\Paginator ) && $vo['data']['avatar']->isEmpty()))): ?>
                                        <img src="<?php echo htmlentities($vo['data']['avatar']); ?>" width="150" height="150"/>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if(!(empty($vo['data']['cover']) || (($vo['data']['cover'] instanceof \think\Collection || $vo['data']['cover'] instanceof \think\Paginator ) && $vo['data']['cover']->isEmpty()))): ?>
                                        <img src="<?php echo htmlentities($vo['data']['cover']); ?>" height="150"/>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if($audit_status === ''): switch($vo['audit_status']): case "0": ?>
                                            待审核
                                    ...
                                    处理：<?php if($vo['handle_time'] != '0'): ?><?php echo htmlentities(time_format($vo['handle_time'],'未处理')); else: ?>未处理<?php endif; if($vo['audit_status'] != '0'): ?>
                                        <br/>详情：<?php echo htmlentities($vo['handle_desc']); endif; ?>
                                </td>
                                <td>
                                    <?php switch($vo['audit_status']): case "0": if(check_auth('admin:user_data_deal:check',AUTH_UID)): ?>
                                                <a data-id="id:<?php echo htmlentities($vo['id']); ?>" poplink="userdata_handler"
                                                href="javascript:;">处理</a><br/>
                                            <?php endif; ?>
                                            <a data-query="id=<?php echo htmlentities($vo['id']); ?>&type=viewback" poplink="task_transfer_box"
                                            href="javascript:;">转交</a>
                                        <?php break; case "1": break; case "2": ?>
                                            原因：<?php echo htmlentities($vo['audit_remark']); break; endswitch; ?>
                                </td>
                            </tr>
...
<div dom-key="work_box" popbox="work_box" class="work_box layer_box" title="任务设置"
    ...
    <div title="用户资料审核" class="layer_box userdata_handler pa_10" dom-key="userdata_handler" popbox-action="<?php echo url('user_data_deal/handler'); ?>">
