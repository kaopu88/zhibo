...
<head lang="en">
    ...
    <script>
        var getInfoUrl = '<?php echo url("category/get_info"); ?>',
            addTypeUrl = '<?php echo url("category/add"); ?>',
            editTypeUrl = '<?php echo url("category/edit"); ?>',
            pid = '<?php echo htmlentities($_pid); ?>';
    </script>
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/category/index.js?v=<?php echo config('upload.resource_version'); ?>"></script>
    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('admin:category:add',AUTH_UID)): ?>
                    <li><a href="javascript:;" class="base_button base_button_s add_btn">新增</a></li>
                <?php endif; if(check_auth('admin:category:delete',AUTH_UID)): ?>
                    <li>
                        <a href="<?php echo url('category/delete'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a>
                    </li>
                <?php endif; if(check_auth('admin:setting:data_update',AUTH_UID)): ?>
                    <li style="line-height: 30px;font-size: 12px;padding-left: 10px;">
                        <a data-to="category:<?php echo htmlentities($data_v); ?>" class="update_data_version" href="javascript:;"><?php echo $data_v=='last'?'已更新至最新版本':'更新至:'.$data_v; ?></a>
                    </li>
                <?php endif; ?>
            </ul>
            <div class="content_toolbar_search">
                <div class="base_group">
                    ...
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;"
                           value="<?php echo input('keyword'); ?>" placeholder="搜索ID、名称"/>
        ...
        <table class="content_list mt_10 sm_width">
            ...
            <tr>
                <td colspan="9">
                    <?php if(is_array($_path) || $_path instanceof \think\Collection || $_path instanceof \think\Paginator): $i = 0; $__LIST__ = $_path;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pa): $mod = ($i % 2 );++$i;?>
                        <a href="<?php echo url('category/index',array('pid'=>$pa['id'])); ?>"><?php echo htmlentities($pa['name']); ?></a>&nbsp;>&nbsp;
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </td>
            </tr>
            <tr class="thead_tr">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr>
                        ...
                        <td><a href="<?php echo url('category/index',array('pid'=>$vo['id'])); ?>"><?php echo htmlentities($vo['name']); ?></a></td>
                        <td><?php echo htmlentities($vo['mark']); ?></td>
                        <td><?php echo htmlentities($vo['sort']); ?></td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:category:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>"
                                 tgradio-name="status" tgradio="<?php echo url('category/change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td><?php echo htmlentities($vo['child_num']); ?></td>
                        <td><?php echo htmlentities(time_format($vo['create_time'],'','date')); ?></td>
                        <td>
                            <?php if(check_auth('admin:category:update',AUTH_UID)): ?>
                                <a data-id="<?php echo htmlentities($vo['id']); ?>" class="edit_btn" href="javascript:;">编辑</a>
                            <?php endif; if(check_auth('admin:category:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('category/delete',array('id'=>$vo['id'])); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
        ...
        <div class="pageshow mt_10">
            <?php echo htmlspecialchars_decode($_page); ?>
        </div>
    ...
    <div class="pa_10">
        <table class="content_info2">
            <tr>
                ...
                <td>
                    <div typecl="<?php echo url('category/get_tree'); ?>">
                        <input type="hidden" class="type_val_select" name="pid" value="<?php echo input('get.pid'); ?>"/>
                    </div>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input name="mark" class="base_text" type="text"/></td>
            </tr>
            <tr>
