...
<head lang="en">
            ...
            list: [
                {
                    name: 'category_id',
                    title: '类别',
                    get: '<?php echo url("common/get_music_category"); ?>'
                },
                {
                    name: 'languages',
                    title: '语种',
    ...
    <script src="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/js'); ?>/music/index.js?v=<?php echo config('upload.resource_version'); ?>"></script>
            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    ...
                    <div class="filter_search">
                        <input placeholder="作者ID、昵称" type="text" name="user_keyword" value="<?php echo input('user_keyword'); ?>"/>
                        <input placeholder="ID、歌曲标题" type="text" name="keyword" value="<?php echo input('keyword'); ?>"/>
                        <button class="filter_search_submit">搜索</button>
                    </div>
                    <div class="clear"></div>
                </div>
            </div>
            <input type="hidden" name="languages" value="<?php echo htmlentities($get['languages']); ?>"/>
            <input type="hidden" name="channel" value="<?php echo htmlentities($get['channel']); ?>"/>
            <input type="hidden" name="status" value="<?php echo htmlentities($get['status']); ?>"/>
            <input type="hidden" name="category_id" value="<?php echo htmlentities($get['category_id']); ?>"/>
        </div>
        ...
        <div class="table_slide">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <div class="thumb">
                                <a href="<?php echo htmlentities($vo['link']); ?>"
                                   target="_blank"
                                   class="thumb_img thumb_img_avatar">
                                    <img src="<?php echo img_url($vo['image'],'200_200','avatar'); ?>"/>
                                </a>
                                <p class="thumb_info">
                                    <a href="<?php echo htmlentities($vo['link']); ?>" target="_blank">
                                        <?php if($vo['is_original'] == '1'): ?>
                                            <span class="fc_red">[原创]</span><br/>
                                        <?php endif; ?>
                                        <?php echo htmlentities($vo['title']); ?><br/>
                                        <a href="<?php echo htmlentities($vo['lrc_link']); ?>">歌词链接</a>
                                    </a>
                                </p>
                            </div>
                        </td>
    ...
    <p class="thumb_info">
        <a href="<?php echo url('user/detail',['user_id'=>$vo['user']['user_id']]); ?>">
            ...
            <?php echo htmlentities((str_hide($vo['user']['phone'],3,4) ?: '未绑定')); ?>
                        ...
                        <td>
                            <?php switch($vo['channel']): case "baidu": ?>
                                    百度
                                <?php break; case "xiaomi": ?>
                                    小米
                                <?php break; case "local": ?>
                                    本地
                                <?php break; endswitch; ?>
                        </td>
                        <td>
                            专辑：<a href="javascript:;"><?php echo htmlentities($vo['album']); ?></a><br/>
                            歌手：<a href="javascript:;"><?php echo htmlentities($vo['singer']); ?></a><br/>
                            语种：<a href="javascript:;"><?php echo htmlentities($vo['languages']); ?></a><br/>
                            风格：<a href="javascript:;"><?php echo htmlentities($vo['style_title']); ?></a><br/>
                            所属公司：<a href="javascript:;"><?php echo htmlentities($vo['company']); ?></a>
                        </td>
                        <td>
                            时长：<a href="javascript:;"><?php echo htmlentities($vo['duration']); ?></a><br/>
                            大小：<a href="javascript:;"><?php echo htmlentities($vo['size']); ?></a><br/>
                            码率：<a href="javascript:;"><?php echo htmlentities($vo['all_bitrate']); ?></a><br/>
                            当前码率：<a href="javascript:;"><?php echo htmlentities($vo['bitrate']); ?></a><br/>
                            格式：<a href="javascript:;"><?php echo htmlentities($vo['ext']); ?></a><br/>
                            标签：<a href="javascript:;"><?php echo htmlentities($vo['tag']); ?></a>
                        </td>
                        <td>
                            使用量：<a href="javascript:;"><?php echo htmlentities($vo['use_num']); ?></a><br/>
                            收藏量：<a href="javascript:;"><?php echo htmlentities($vo['collect_num']); ?></a><br/>
                            歌词举报次数：<a href="javascript:;"><?php echo htmlentities($vo['lrc_report']); ?></a><br/>
                            相关描述：<a href="javascript:;"><?php echo htmlentities($vo['desc']); ?></a>
                        </td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:music:update')?'0':'1'; ?>" tgradio-on="1"
                                 tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>"
                                 tgradio-on-name="上架" tgradio-off-name="下架"
                                 tgradio-name="status"
                                 tgradio="<?php echo url('music/change_status',['id'=>$vo['id']]); ?>"></div>
                        </td>
                        <td>
                            发表时间：<?php echo htmlentities(time_format($vo['release_time'],'无','date')); ?><br/>
                            创作时间：<?php echo htmlentities(time_format($vo['create_time'],'无','date')); ?>
                        </td>
                        <td>
                            <?php if(check_auth('admin:music:update',AUTH_UID)): ?>
                                <a href="<?php echo url('edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a>
                            <?php endif; if(check_auth('admin:music:delete',AUTH_UID)): ?>
                                <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('delete',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
