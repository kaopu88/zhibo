            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <?php if(check_auth('admin:agent:add',AUTH_UID)): ?>
                        <a href="<?php echo url('agent/add',array('aid'=>$aid)); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a>
...
<!--                        <td>-->
<!--                            <?php echo htmlentities($vo['contact_name']); ?> <span class="fc_green">v</span><br/>-->
<!--                            <?php echo htmlentities($vo['contact_phone']); ?>-->
<!--                        </td>-->
                        <td>
...
<!--                            <?php echo config('app.product_setting.millet_name'); ?>：<?php echo htmlentities($vo['total_millet']); ?><br/>-->
...
<!--                        </td>-->
                        ...
                        <td>
                            <?php switch($vo['applystatus']): case "0": ?>
                                    <div tgradio-not="<?php echo check_auth('admin:agent:update')?'0':'1'; ?>" tgradio-on="1"
                                         ...
                                         tgradio="<?php echo url('agent/change_status',array('id'=>$vo['id'])); ?>"></div> <br/>
                                <?php break; case "1": ?>
                                    等待审核
                                <?php break; case "2": ?>
                                    审核未通过
                        ...
                        <td>
                            <?php if(!(empty($vo['root_id']) || (($vo['root_id'] instanceof \think\Collection || $vo['root_id'] instanceof \think\Paginator ) && $vo['root_id']->isEmpty()))): if(check_auth('admin:agent:update',AUTH_UID)): ?>
                                    <a href="<?php echo url('agent/edit',array('id'=>$vo['id'],'aid'=>$aid)); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑信息</a> <br/>
                                <?php endif; else: if($vo['applystatus'] == '0'): ?><a href="<?php echo url('agent/set_root',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">设置主账号</a> <br/><?php endif; endif; if(check_auth('admin:agent:transfer',AUTH_UID)): if($vo['applystatus'] == '0'): ?><a target="_blank" href="<?php echo url('agent/transfer',array('id'=>$vo['id'])); ?>">传送后台</a><br/><?php endif; endif; if(check_auth('admin:agent:update',AUTH_UID)): if($vo['applystatus'] == '0'): ?><a href="<?php echo url('agent/manager',array('id'=>$vo['id'])); ?>">人员管理</a><br/><?php endif; endif; if(check_auth('admin:agent:delete',AUTH_UID)): ?>
