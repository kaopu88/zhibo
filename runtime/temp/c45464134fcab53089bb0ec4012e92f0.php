...
<head lang="en">
    ...
    <link rel="stylesheet" type="text/css" href="<?php echo ('/static/'.strtolower(\think\facade\Request::module()).'/css'); ?>/annex/index.css?v=<?php echo config('upload.resource_version'); ?>"/>
    <style>
        .thumb .thumb_img {
            flex: none;
            width: 100px;
  ...
  <div class="sub_catalog"></div>
    ...
    <div class="pa_20">
        ...
        <div class="filter_box mt_10">
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <a href="<?php echo url('index'); ?>" class="base_button base_button_s base_button_gray" title="首页"><i class="icon-home"></i></a>
                    <?php echo htmlentities($prefix); ?>
                    <div class="clear"></div>
                </div>
            </div>
        </div>
        ...
        <div class="table_slide">
            ...
            <tbody>
            <?php if(!empty($_list['prefixes']) ||  !empty($_list['fileList'])): if(is_array($_list['prefixes']) || $_list['prefixes'] instanceof \think\Collection || $_list['prefixes'] instanceof \think\Paginator): $i = 0; $__LIST__ = $_list['prefixes'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr>
                        <td><input class="list_id" type="checkbox" name="ids[]" value="<?php echo htmlentities($vo); ?>"/></td>
                        <td><a href="<?php echo url('index',['prefix'=>$key]); ?>"><?php echo htmlentities($vo); ?></a></td>
                        <td>目录</td>
                        <td>-</td>
                        <td></td>
                    </tr>
                <?php endforeach; endif; else: echo "" ;endif; if(is_array($_list['fileList']) || $_list['fileList'] instanceof \think\Collection || $_list['fileList'] instanceof \think\Paginator): $i = 0; $__LIST__ = $_list['fileList'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
                    <tr>
                        <td><input class="list_id" type="checkbox" name="ids[]" value="<?php echo htmlentities($v['key']); ?>"/></td>
                        <td>
                            <?php if($v['is_img'] == 1): ?>
                                <div class="thumb">
                                    <a rel="thumb" href="<?php echo img_url($_list['base'].'/'.$v['key'],'','thumb'); ?>" class="thumb thumb_img user_base_avatar fancybox" alt="">
                                        <img src="<?php echo img_url($_list['base'].'/'.$v['key'],'200_200','thumb'); ?>"/>
                                    </a>
                                </div>
                                <?php else: ?>
                                <a href="<?php echo htmlentities($_list['base']); ?>/<?php echo htmlentities($v['key']); ?>" target="_blank"><?php echo htmlentities(basename($v['key'])); ?></a>
                            <?php endif; ?>
                        </td>
                        <td><?php echo htmlentities(format_bytes($v['fsize'])); ?></td>
                        <td><?php echo htmlentities(time_format(intval($v['putTime']/10000000))); ?></td>
                        <td>
                            <a ajax="get" href="<?php echo url('delFile',['file'=>urlencode($v['key'])]); ?>">删除</a>
                            <a href="<?php echo url('downfile',['file'=>urlencode($v['key']),'size'=>$v['fsize']]); ?>" target="_blank">下载</a>
                        </td>
                    </tr>
