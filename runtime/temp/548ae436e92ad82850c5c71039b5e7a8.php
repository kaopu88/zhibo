    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                ...
                <!--<?php if(check_auth('admin:lottery:lottery_add',AUTH_UID)): ?>
                    <li><a href="<?php echo url('lottery_add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>"
                           class="base_button base_button_s">新增</a>
                    </li>
                <?php endif; if(check_auth('admin:lottery:lottery_delete',AUTH_UID)): ?>
                    <li><a href="<?php echo url('lottery_delete'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a></li>
                <?php endif; ?>-->
        ...
        <table class="content_list mt_10">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:lottery:lottery_edit')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>

                        <td>
                        <a href="<?php echo url('lottery_edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a><br>
                        <!--<a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('lottery_delete',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">删除</a><br>-->
                        <a style="color: #f00" href="<?php echo url('LotteryGift/index',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">奖品管理</a><br/>
