    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('admin:admin:add',AUTH_UID)): ?>
                    <li><a href="<?php echo url('admin/add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a> </li>
                <?php endif; if(check_auth('admin:admin:delete',AUTH_UID)): ?>
                    <li><a href="<?php echo url('admin/del'); ?>" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_red base_button_s">删除</a></li>
                <?php endif; ?>
            </ul>
            <div class="content_toolbar_search">
                <div class="base_group">
                    ...
                    <div class="modal_select modal_select_s">
                        <span class="modal_select_text"></span>
                        <input name="gid" type="hidden" class="modal_select_value finder" value="<?php echo input('gid'); ?>"/>
                        <ul class="modal_select_list">
                            <li class="modal_select_option" value="">全部</li>
                            <?php if(is_array($group_list) || $group_list instanceof \think\Collection || $group_list instanceof \think\Paginator): $i = 0; $__LIST__ = $group_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gp): $mod = ($i % 2 );++$i;?>
                                <li class="modal_select_option" value="<?php echo htmlentities($gp['id']); ?>"><?php echo htmlentities($gp['name']); ?></li>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
                    <input name="keyword" class="base_text base_text_s finder" style="width:180px;" value="<?php echo input('keyword'); ?>" placeholder="搜索ID、手机号、姓名" />
        ...
        <table class="content_list mt_10 sm_width">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr>
                        ...
                        <td>
                            <?php echo htmlentities($vo['username']); ?>（<?php echo htmlentities((isset($vo['phone']) && ($vo['phone'] !== '')?$vo['phone']:'未填写')); ?>）
                        </td>
                        <td><?php echo htmlentities($vo['realname']); ?></td>
                        <td>
                            <?php if($vo['is_root'] == '1'): ?>
                                <span style="color: orange;">超级管理员</span>
                                <?php else: if(is_array($vo['group_list']) || $vo['group_list'] instanceof \think\Collection || $vo['group_list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['group_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?>
                                    <span><?php echo htmlentities($group['name']); ?></span>&nbsp;
                                <?php endforeach; endif; else: echo "" ;endif; endif; ?>
                        </td>
                        <td>
                            <?php if($vo['is_root'] == '1'): ?>
                                <div tgradio="" tgradio-not="1"   tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status"></div>
                                <?php else: ?>
                                <div tgradio-not="<?php echo check_auth('admin:admin:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('admin/change_status',array('id'=>$vo['id'])); ?>"></div>
                            <?php endif; ?>
                        </td>
                        <td><?php echo htmlentities(time_before($vo['login_time'],'前')); ?></td>
                        <td >
                            <?php if($vo['is_root'] == '1'): else: if(check_auth('admin:admin:update',AUTH_UID)): ?>
                                    <a href="<?php echo url('admin/edit',array('id'=>$vo['id'])); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>">编辑</a>
                                <?php endif; if(check_auth('admin:admin:delete',AUTH_UID)): ?>
                                    <a class="fc_red" ajax-confirm ajax="get" href="<?php echo url('admin/del',array('id'=>$vo['id'])); ?>">删除</a>
                                <?php endif; endif; ?>
                        </td>
                    </tr>
