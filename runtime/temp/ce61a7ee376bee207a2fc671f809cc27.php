    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <?php if(check_auth('admin:ad_space:add',AUTH_UID)): ?>
                    <li><a href="<?php echo url('add'); ?>?<?php echo ('redirect='.urlencode(\think\facade\Request::url())); ?>" class="base_button base_button_s">新增</a></li>
                <?php endif; if(check_auth('admin:ad_space:delete',AUTH_UID)): ?>
        ...
        <table class="content_list mt_10 md_width">
            ...
            <?php if(!(empty($_list) || (($_list instanceof \think\Collection || $_list instanceof \think\Paginator ) && $_list->isEmpty()))): if(is_array($_list) || $_list instanceof \think\Collection || $_list instanceof \think\Paginator): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr data-id="<?php echo htmlentities($vo['id']); ?>">
                        ...
                        <td><?php echo enum_attr('ad_space_types',$vo['type'],'name'); ?></td>
                        <td><a href="<?php echo url('ad_content/index',['space_id'=>$vo['id']]); ?>"><?php echo htmlentities($vo['count']); ?>&nbsp;</a>/<?php echo htmlentities($vo['length']); ?>
                        </td>
                        <td><?php echo htmlentities($vo['pv']); ?></td>
                        <td>
                            <div tgradio-not="<?php echo check_auth('admin:ad_space:update')?'0':'1'; ?>" tgradio-on="1" tgradio-off="0" tgradio-value="<?php echo htmlentities($vo['status']); ?>" tgradio-name="status" tgradio="<?php echo url('ad_space/change_status',array('id'=>$vo['id'])); ?>"></div>
                        </td>
                        <td><?php echo htmlentities(time_format($vo['create_time'])); ?></td>
                        <td>
                            <?php if(check_auth('admin:ad_space:update',AUTH_UID)): ?>
