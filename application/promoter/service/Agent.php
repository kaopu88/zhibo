...
class Agent extends \bxkj_module\service\Agent
{
    ...
    public function check_agent($userId)
    {
        $agentId = Db::name('user')->where(['user_id' => $userId])->value('agent_id');
        $array = [AGENT_ID];
        $where = [['pid', '=', AGENT_ID]];
        $agentIds = Db::name('agent')->field('id')->where($where)->select();
        if ($agentIds)
        {
        ...
        if (in_array($agentId,$array))
        {
            return true;
        }
        return $this->setError('无权进行此操作！');
    }
