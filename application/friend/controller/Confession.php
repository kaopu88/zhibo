...
class Confession extends Controller
{
    ...
    {
        $this->checkAuth('friend:Confession:index');
        $get        = input();
        $msgComment = new  FriendCircleConfessionEvaluate();
        $total      = $msgComment->getTotal($get);
        $page       = $this->pageshow($total);
        $list       = $msgComment->getList($get, $page->firstRow, $page->listRows);
        foreach ($list as $k=>$v){
            $list[$k]['imgs'] =array_filter(explode(',',$v['imgs'])) ?array_filter(explode(',',$v['imgs'])):'';
        }
    ...
    public function del()
    {
        $this->checkAuth('friend:Confession:delete');
        $ids = get_request_ids();
        if (empty($ids)) $this->error('请选择记录');
        $msgComment = new FriendCircleConfessionEvaluate();
        $num        = $msgComment->del($ids);
        if (!$num) $this->error('删除失败');
        alog("friend.evaluate.del", "删除表白留言 ID：".implode(",", $ids));
        $count = count($ids);
    ...
    public function add()
    {
        $this->checkAuth('friend:Confession:add');
        if (Request::isGet()) {
            $info = [];
            $get  = input();
            ...
            $msgCommentEval = new FriendCircleConfessionEvaluate();
            $post           = input();
    ...
    public function edit()
    {
        if (Request::isGet()) {
            $id   = input('id');
            $info = Db::name('FriendCircleConfessionEvaluate')->where('id', $id)->find();
            if (empty($info)) $this->error('信息不存在');
            $this->assign('_info', $info);
            return $this->fetch('add');
        } else {
            $msgCommentE = new FriendCircleConfessionEvaluate();
            $post        = input();
    ...
    {
        $this->checkAuth('friend:Confession:change_status');
        $ids = get_request_ids();
        if (empty($ids)) $this->error('请选择记录');
        $status = input('status');
    ...
    public function evaluateExamine(){
        $this->checkAuth('friend:Confession:msgExamine');
        $get = input();
        $get['aid'] = AID;
