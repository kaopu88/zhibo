...
class LotteryType extends Controller
{
    ...
    {
        $this->checkAuth('lottery:lottery_type:select');
        $get = input();
        $lotteryTypeService = new \app\lottery\service\LotteryType();
    ...
    {
       $this->checkAuth('lottery:lottery_type:category_add');
        if (Request::isGet()) {
            return $this->fetch();
        } else {
            $lotteryTypeService = new \app\lottery\service\LotteryType();
    ...
    {
        $this->checkAuth('lottery:lottery_type:category_edit');
        if (Request::isGet()) {
            $id = input('id');
    ...
    {
        $this->checkAuth('lottery:lottery_type:delete');
        $ids = get_request_ids();
    ...
    {
        $this->checkAuth('lottery:lottery_type:update');
        $ids = get_request_ids();
        if (empty($ids)) $this->error('请选择记录');
        $status = input('status');
