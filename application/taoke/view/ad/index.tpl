        ...
        <table class="content_list mt_10">
            ...
            <notempty name="_list">
                <volist name="_list" id="vo">
                    <tr data-id="{$vo.ad_id}">
                        <td><input class="list_id" type="checkbox" name="ids[]" value="{$vo.ad_id}"/></td>
                        <td>{$vo.ad_id}</td>
                        <td>{$vo.title}</td>
                        <td>{$vo.desc}</td>
                        <td>
                        ...
                        <td>
                            <div tgradio-not="{:check_auth('taoke:ad:update')?'0':'1'}" tgradio-on="1" tgradio-off="0"
                                 tgradio-value="{$vo.status}" tgradio-name="status" tgradio="{:url('changeStatus',array('id'=>$vo['ad_id']))}"></div>
                        </td>
                        <td>
                            <auth rules="taoke:ad:update">
                                <a href="{:url('edit',array('id'=>$vo['ad_id']))}?__JUMP__">编辑</a> |
                            </auth>
                            <auth rules="taoke:ad:delete">
                                <a class="fc_red" ajax-confirm ajax="get" href="{:url('del',array('id'=>$vo['ad_id']))}?__JUMP__">删除</a>
                            </auth>
                        </td>
                    </tr>
                </volist>
