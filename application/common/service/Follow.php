...
class Follow extends Service
{
    ...
    public function addFollow($user_id, $is_return=0, $exists=[])
    {
        ...
        $res = Db::name('follow')->insert(['user_id'=>USERID, 'follow_id'=>$user_id, 'type'=> 1, 'create_time'=>$nowTime]);
    ...
    {
        $res = Db::name('follow')->where('user_id=? and follow_id=?', USERID, $user_id)->delete();
    ...
    //粉丝列表
    public function fansList($user_id, $p=1)
    {
        $start = empty($p) ? 0 : ($p-1)*PAGE_LIMIT;
    ...
    public function currentFollow($user_id, $offset, $length)
    {
        ...
        foreach ($lists as &$value)
        {
            ...
            if ($value['is_live'])
            {
                ...
                $value['jump'] = getJump('enter_room', ['room_id' => $room['room_id']]);
            }
...
//        $now = time();
    ...
    public function getFollowInfo($user_id, $followBuguid)
    {
        ...
        if (!$redis->exists($key)) {
            ...
            $followList = $followDb->field('follow_id,create_time')->where($where)->select();
    ...
    public function getNewPublish($user_id, $offset=0, $length=10)
    {
        ...
        $sql = sprintf('SELECT v.* FROM '.$prefix.'follow follow INNER JOIN '.$prefix.'video v ON follow.follow_id=v.user_id WHERE follow.user_id=%s ORDER BY v.id desc LIMIT %s, %s', $user_id, $offset, $length);
