...
class User extends \bxkj_module\service\User
{
    ...
    public function getMyTotal($get)
    {
        $this->db = Db::name('user');
        $this->setMyWhere($get);
        $count = $this->db->count();
    ...
    public function getList($get, $offset = 0, $length = 20)
    {
        ...
        $promoterList = $this->getUsersByIds($promoterIds);
        ...
        foreach ($result as &$item) {
            ...
            $userPromoter = Db::name('promoter')->field('agent_id')->where(['user_id' => $item['user_id']])->find();
    ...
    public function getMyList($get, $offset = 0, $length = 20)
    {
        $this->db = Db::name('user');
        $this->setMyWhere($get)->setOrder($get);
        $result = $this->db->limit($offset, $length)->select();
        foreach ($result as &$item) {
            $item = array_merge($item, ExpLevel::getLevelInfo($item['level']));
            self::parseUser($item);
    ...
    protected function setWhere($get)
    {
        ...
        if ($get['agent_id']) {
            Agent::agentWhere($where, ['agent_id' => $get['agent_id']], 'pr.');
        } else {
        ...
        if (!isset($get['agent_id'])) {
            $this->db->whereIn('pr.agent_id', $agentIds);
        }
        return $this;
    }

    protected function setMyWhere($get)
    {
        $this->db->alias('user');
        $this->db->join('__PROMOTION_RELATION__ pr', 'user.user_id=pr.user_id');
        $where = [['user.delete_time', 'null']];
