...
<body class="task_setting">
	...
	<div class="mui-content <if condition='$task_type eq 0'>mui-hidden</if>" id="user-task-content">
		<ul class="mui-table-view">
			<foreach name="taskItem.task_detail" id="vo" key="i">
				<li class="mui-table-view-cell mui-collapse">
					<img class="mui-media-object mui-pull-left" src="{$vo.icon}">
					<div class="mui-media-body">
						<p class='mui-ellipsis'>{$vo.title} <span class="mui-badge mui-badge-success">{$vo.unit}</span></p>
						<input class="mui-input-numbox input" data-init-value="{$vo.num}" name="{$vo.name}" data-numbox-min="{$vo.min}" type="number" value="{$vo.num}" onchange="input(this)" <if condition='$task_type eq 1'>disabled="disabled"</if> style="<if condition='$i eq 2'>width: 28% !important;</if>"/>
					</div>
				</li>
			</foreach>
		</ul>
		<if condition='$task_type eq 0'>
			<div class="mui-text-center submit" style="margin-top: 170px;">
				<img id="submit" src="__H5__/images/setting/sub.png" alt="" style="width: 60%;">
			</div>
		</if>
	</div>
