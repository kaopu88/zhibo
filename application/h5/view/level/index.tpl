...
<link rel="stylesheet" type="text/css" href="__H5__/css/swiper-3.4.2.min.css?v=__RV__">
</head>
...
<style type="text/css">
	*{
	...
	#nav{
		...
		height:0.88rem;
		background:#58BFBD;
		background-image:url(__H5__/images/level/return_back.png);
...
<div class="page">
	<div class="headPic">
		<img src="<if condition="!empty($user['avatar']) eq true">{$user['avatar']}<else />http://live.yunbaozhibo.com/api/public/upload/avatar/default_thumb.jpg</if>" class="uavatar">
		<div class="headHover">
			<p class="level">Lv.{$user['level']}</p>
		</div>
	</div>
	<p class="levelDesc">当前累计经验<span class="alert">{$user['exp']}</span>，离升级还差<span class="alert">{$cha}</span></p>
	<div class="levelDetail">
		<div class="currentLevel alert">
			Lv.{$user['level']}
		</div>
		<div class="levelLine">
			<div class="getLev" style="width: {$rate}%"></div>
		</div>
		<div class="nextLevel">Lv.{$user['level']+1}</div>
	</div>
</div>
...
<script type="text/javascript" src="__H5__/js/swiper-3.4.2.min.js?v=__RV__"></script>
<script type="text/javascript">
