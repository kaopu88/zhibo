            ...
            <div class="filter_options">
                ...
                <div class="filter_order">
                    <auth rules="admin:agent:add">
                        <a href="{:url('agent/add',array('aid'=>$aid))}?__JUMP__" class="base_button base_button_s">新增</a>
                    </auth>
...
<!--                        <td>-->
<!--                            客消：{$vo.total_cons}<br/>-->
<!--                            {:config('app.product_setting.millet_name')}：{$vo.total_millet}<br/>-->
<!--                            拉新：{$vo.total_fans}-->
<!--                        </td>-->
                        <td>
                            {$vo.province_name}<br>
                            ...
                            <switch name="vo['applystatus']">
                                <case value="0">
                                    <div tgradio-not="{:check_auth('admin:agent:update')?'0':'1'}" tgradio-on="1"
                                         ...
                                         tgradio="{:url('agent/change_status',array('id'=>$vo['id']))}"></div> <br/>
                                </case>
                                <case value="1">
                                    等待审核
                                </case>
                        ...
                        <td>
                            <notempty name="vo['root_id']">
                                <auth rules="admin:agent:update">
                                    <a href="{:url('agent/edit',array('id'=>$vo['id'],'aid'=>$aid))}?__JUMP__">编辑信息</a> <br/>
                                </auth>
                                <else/>
                                <eq name="vo['applystatus']" value="0"><a href="{:url('agent/set_root',array('id'=>$vo['id']))}?__JUMP__">设置主账号</a> <br/></eq>
                            </notempty>
                            <auth rules="admin:agent:transfer">
                                <eq name="vo['applystatus']" value="0"><a target="_blank" href="{:url('agent/transfer',array('id'=>$vo['id']))}">传送后台</a><br/></eq>
                            </auth>
                            <auth rules="admin:agent:update">
                                <eq name="vo['applystatus']" value="0"><a href="{:url('agent/manager',array('id'=>$vo['id']))}">人员管理</a><br/></eq>
                            </auth>
                            <auth rules="admin:agent:delete">
