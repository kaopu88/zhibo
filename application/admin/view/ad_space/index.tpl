            ...
            <notempty name="_list">
                <volist name="_list" id="vo">
                    <tr data-id="{$vo.id}">
                        ...
                        <td> {$vo.name}</td>
                        <td>{$vo.mark}</td>
                        <td>{:enum_attr('ad_space_types',$vo['type'],'name')}</td>
                        <td><a href="{:url('ad_content/index',['space_id'=>$vo['id']])}">{$vo.count}&nbsp;</a>/{$vo.length}
                        </td>
                        <td>{$vo.pv}</td>
                        <td>
                            <div tgradio-not="{:check_auth('admin:ad_space:update')?'0':'1'}" tgradio-on="1" tgradio-off="0" tgradio-value="{$vo.status}" tgradio-name="status" tgradio="{:url('ad_space/change_status',array('id'=>$vo['id']))}"></div>
                        </td>
                        <td>{$vo.create_time|time_format}</td>
                        <td>
                            <auth rules="admin:ad_space:update">
