    ...
    <div class="pa_20">
        ...
        <div class="content_toolbar mt_10">
            <ul class="content_toolbar_btns">
                <auth rules="admin:recommend_space:add">
                    <li><a href="{:url('add')}?__JUMP__" class="base_button base_button_s">新增</a></li>
                </auth>
                <auth rules="admin:recommend_space:delete">
            ...
            <notempty name="_list">
                <volist name="_list" id="vo">
                    <tr data-id="{$vo.id}">
                        ...
                        <td>{$vo.mark}</td>
                        <td>
                            <a href="{:url('recommend_content/'.$vo['type'],array('rec_id'=>$vo['id']))}">{$vo.count}</a>
                        </td>
                        <td>{:enum_attr('recommend_space_types',$vo['type'],'name')}</td>
                        <td>
                            <div tgradio-not="{:check_auth('admin:recommend_space:update')?'0':'1'}" tgradio-on="1" tgradio-off="0" tgradio-value="{$vo.status}" tgradio-name="status" tgradio="{:url('recommend_space/change_status',array('id'=>$vo['id']))}"></div>
                        </td>
                        <td>{$vo.create_time|time_format}</td>
                        <td>
                            <auth rules="admin:recommend_space:update">
