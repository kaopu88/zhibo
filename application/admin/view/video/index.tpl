                ...
                <div class="filter_order">
                    <div style="float: left">
                        <auth rules="admin:film:add">
                            <a class="base_button base_button_s" target="_blank" href="{:url('video/batchadd')}">上传视频</a>
                        </auth>
                        <auth rules="admin:film:change_status">
                            <div ajax="post" ajax-url="{:url('video/change_status',['status'=>'1'])}"
                                 ajax-target="list_id"
                                 class="base_button base_button_s base_button_gray">上架
                            </div>
                            <div ajax="post" ajax-url="{:url('video/change_status',['status'=>'0'])}"
                                 ajax-target="list_id"
                                 class="base_button base_button_s base_button_gray">下架
                            </div>
                            <auth rules="admin:film:delete">
                                <a href="{:url('video/del')}" ajax="post" ajax-target="list_id" ajax-confirm class="base_button base_button_s base_button_red">删除</a>
                            </auth>
                        </auth>
                    </div>
            ...
            <notempty name="_list">
                <volist name="_list" id="vo">
                    <tr data-id="{$vo.id}">
                        ...
                        <td>
                            <notempty name="vo['goods']">
                                ...
                                <span style="color: #8c8c8c">
                                    <switch name="vo['goods']['shop_type']"><case value="B">来自天猫</case><case value="C">来自淘宝</case><case value="P">来自拼多多</case><case value="J">来自京东</case><case value="Z">来自自营</case></switch>
                                </span>
                                <else/>
                                暂无
                            </notempty>
                        ...
                        <td>
                            播放：{$vo.play_sum} <a poplink="play_sum" data-query="id={$vo.id}" href="javascript:;">调整</a><br/>
                            下载：{$vo.down_sum}<br/>
                            总赞：{$vo.zan_sum} <span class="fc_gray">({$vo.sco_zan_sum})</span> <br/>
                            虚拟赞：{$vo.zan_sum2}  <a poplink="zan_sum2" data-query="id={$vo.id}" href="javascript:;">调整</a><br/>
...
<block name="layer">
    ...
    <include file="video/zan_sum"/>
    <include file="video/play_sum"/>
