...
class Topic extends Service
{
    ...
    private function setOrder($get)
    {
        ...
        if (empty($get['sort'])) {
            ...
            $order['id'] = 'desc';
        } else if ($get['sort'] == 'participate_num' && $get['sort_by']) {
            $order['participate_num'] = $get['sort_by'];
            $order['create_time'] = 'desc';
            $order['id'] = 'desc';
        }
        $this->db->order($order);
    ...
    {
        $data = $this->df->process('add@topic', $inputData)->output();
    ...
    {
        $data = $this->df->process('update@topic', $inputData)->output();
        if ($data === false) return $this->setError($this->df->getError());
        $num = Db::name('topic')->where('id', $data['id'])->update($data);
    ...
    public function delete($ids)
    {
        ...
        if (empty($ids)) return $this->setError('请选择话题');
        $num = Db::name('topic')->whereIn('id', $ids)->delete();
