...
class Vip extends Service
{
    ...
    {
        $data = $this->df->process('add@vip', $inputData)->output();
    ...
    {
        $data = $this->df->process('update@vip', $inputData)->output();
        if ($data === false) return $this->setError($this->df->getError());
        $num = Db::name('vip')->where(array('id' => $data['id']))->update($data);
    ...
    public function changeStatus($ids, $status)
    {
        ...
        return Db::name('vip')->whereIn('id', $ids)->update(['status'=>$status]);
    }
}