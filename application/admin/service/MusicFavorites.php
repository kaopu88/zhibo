...
class MusicFavorites extends Service
{
    public function getTotal($get){
        $this->db = Db::name('music_favorites');
    ...
    {
        $this->db->alias('mf')->join('__USER__ user', 'user.user_id=mf.user_id', 'LEFT');
        $this->db->join('__MUSIC__ m', 'm.id=mf.music_id', 'LEFT');
        $this->db->field('mf.id,mf.user_id,mf.create_time,m.link,m.image,m.title,m.is_original,m.lrc_link');
        return $this;
    }
