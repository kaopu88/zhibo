...
class ExpLevel extends Service
{
    ...
    {
        $data = $this->df->process('add@exp_level', $inputData)->output();
        ...
        $id = Db::name('exp_level')->insertGetId($data);
    ...
    {
        $data = $this->df->process('update@exp_level', $inputData)->output();
        if ($data === false) return $this->setError($this->df->getError());
        $check = $this->check_name($data['levelname'],$data['levelid']);
        if ($check) return $this->setError($check);
        $num = Db::name('exp_level')->where(array('levelid' => $data['levelid']))->update($data);
