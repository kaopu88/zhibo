...
class Lottery extends Controller
{
    ...
    {
        $this->checkAuth('admin:lottery:select');
        $get = input();
        $lotteryService = new \app\admin\service\Lottery();
    ...
    {
       $this->checkAuth('admin:lottery:lottery_add');
        if (Request::isGet()) {
            $where['status'] = '1';
    ...
    {
        $this->checkAuth('admin:lottery:lottery_edit');
        if (Request::isGet()) {
            $id = input('id');
    ...
    {
        $this->checkAuth('admin:lottery:lottery_delete');
        $ids = get_request_ids();
    ...
    {
        $this->checkAuth('admin:lottery:lottery_update');
        $ids = get_request_ids();
        if (empty($ids)) $this->error('请选择记录');
        $status = input('status');
